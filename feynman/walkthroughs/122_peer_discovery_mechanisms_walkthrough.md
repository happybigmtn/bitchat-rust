...
pub struct PeerDiscoveryManager {
    ...
    // Future Work: mDNS local discovery (not yet implemented)
...
impl PeerDiscoveryManager {
    pub async fn start_discovery(&self) -> Result<(), DiscoveryError> {
        ...
        // let mdns_task = self.mdns_discovery.start_broadcasting(); // Future Work
...
```rust
// Future Work: mDNS local discovery (no current dedicated module)  
    ...
    pub async fn execute_discovery_strategy(&self, strategy: DiscoveryStrategy) -> Result<(), DiscoveryError> {
        match strategy {
            DiscoveryStrategy::LocalFirst => {
                ...
                // tokio::spawn(self.mdns_discovery.start_broadcasting()); // Future Work
