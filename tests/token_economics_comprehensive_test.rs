...
use bitcraps::{
    TokenLedger, TokenEconomics, TreasuryManager, ContractManager,
    BlockchainNetwork, EconomicsConfig, TreasuryConfig, CompoundingFrequency,
    CrapTokens, Result, Error,
};
...
async fn test_complete_token_economics_flow() -> Result<()> {
    ...
    let amm_id = treasury.create_amm_pool(token_a_reserve, token_b_reserve, swap_fee).await?;
    ...
    let output = treasury.swap_tokens(amm_id, true, swap_input, min_output, trader).await?;
    ...
    let low_fee = economics.calculate_dynamic_fee(
        bitcraps::economics::FeeCategory::Transfer, 
        false
    ).await;
    
    economics.update_fee_structure(3000).await?; // High volume
    let high_fee = economics.calculate_dynamic_fee(
        bitcraps::economics::FeeCategory::Transfer, 
        false
    ).await;
    ...
    economics.burn_tokens(burn_amount, "Quarterly burn".to_string()).await?;
    ...
    let eth_contract = contracts.deploy_token_contract(
    ...
    let staking_contract = contracts.deploy_staking_contract(
        BlockchainNetwork::Ethereum,
        eth_contract.clone(),
        eth_contract.clone(),
        reward_rates,
    ...
    let bridge_contract = contracts.deploy_bridge_contract(
        BlockchainNetwork::Ethereum,
        vec![BlockchainNetwork::BinanceSmartChain, BlockchainNetwork::Polygon],
    ...
    let bridge_operation = contracts.bridge_tokens(
        bridge_contract,
    ...
    let (price, timestamp) = contracts.get_oracle_price(
        "0xoracle1234567890123456789012345678901234567890"
    ).await?;
    ...
    for i in 0..100 {
        ...
        let _fee = economics.calculate_dynamic_fee(
            bitcraps::economics::FeeCategory::Transfer, 
            i % 2 == 0
        ).await;
    ...
    println!("✓ Performance test: 100 operations completed in {:?}", elapsed);
    ...
    let bridge_op = contracts.bridge_tokens(
        bridge_contract,
...
async fn test_integration_stress_scenarios() -> Result<()> {
    ...
    treasury.initialize_treasury(CrapTokens::from(5_000_000_000_000)).await?; // 5M CRAP
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            treasury_clone.swap_tokens(amm_id, true, swap_amount, min_output, trader).await
        });
    ...
    println!("✓ Stress test 2 (rapid AMM swaps) passed with {}/20 successful swaps", successful_swaps);
    ...
    for _i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            economics_clone.calculate_staking_rewards().await
        });
