...
impl TestParticipant {
    ...
    async fn commit_phase(&mut self, value: [u8; 32], nonce: [u8; 32]) -> Option<SignedMessage<RandomnessCommit>> {
    ...
    async fn reveal_phase(&mut self, value: [u8; 32], nonce: [u8; 32]) -> Option<SignedMessage<RandomnessReveal>> {
...
async fn simulate_consensus_round(
    ...
    for _ in 0..honest_count {
        participants.push(TestParticipant::new(treasury.clone(), false, AttackType::None));
    }
    ...
    let proposer_idx = participants.iter()
    ...
    let proposal = participants[proposer_idx].engine.create_proposal(game_id, settlements)
...
async fn test_consensus_with_minority_withholding_commits() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Consensus should succeed with 70% participation");
}
...
async fn test_consensus_with_minority_withholding_reveals() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Consensus should succeed despite withheld reveals");
}
...
async fn test_consensus_with_invalid_reveals() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Consensus should succeed, invalid reveals ignored");
}
...
async fn test_consensus_with_minority_rejecting_proposals() {
    ...
    assert!(result.is_ok());
    assert!(result.unwrap(), "Consensus should succeed with 70% approval");
}
...
async fn test_consensus_with_majority_attack_fails() {
    ...
    assert!(result.is_ok());
    assert!(!result.unwrap(), "Consensus should fail with majority attack");
}
...
async fn test_reputation_penalties_for_non_participation() {
    ...
    assert!(trust_level < 0.5, "Trust level should be low after repeated failures");
    ...
    assert!(!reputation_manager.can_participate(&malicious_id), 
...
async fn test_dispute_resolution_for_cheating() {
    ...
    let dispute_id = reputation_manager.raise_dispute(
    ...
    for voter in voters {
        ...
        reputation_manager.vote_on_dispute(dispute_id, voter, vote)
...
async fn test_forced_settlement_on_timeout() {
    ...
    assert!(engine.is_stuck(), "Engine should detect stuck state after timeout");
...
async fn test_treasury_locks_during_consensus() {
    ...
    treasury.process_bet(game_id, bet_amount, max_payout)
    ...
    treasury.settle_bet(game_id, max_payout, bitcraps::protocol::CrapTokens::from(1500))
...
async fn test_consensus_recovery_from_network_partition() {
    ...
    for _ in 0..3 {
        group1.push(TestParticipant::new(treasury.clone(), false, AttackType::None));
    }
    
    for _ in 0..2 {
        group2.push(TestParticipant::new(treasury.clone(), false, AttackType::None));
    }
