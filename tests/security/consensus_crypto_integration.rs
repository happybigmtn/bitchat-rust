...
use bitcraps::protocol::{PeerId, Signature, Hash256};
...
async fn test_consensus_real_ed25519_signatures() {
    ...
    assert_eq!(signature.signature.len(), 64, "Ed25519 signature should be 64 bytes");
    assert_eq!(signature.public_key.len(), 32, "Ed25519 public key should be 32 bytes");
    assert_ne!(signature.signature, vec![0u8; 64], "Signature should not be all zeros");
    assert_ne!(signature.public_key, vec![0u8; 32], "Public key should not be all zeros");
    
    // Verify signature validates correctly
    let is_valid = SecureKeystore::verify_secure_signature(
        message,
        &signature,
...
async fn test_osrng_cryptographic_quality() {
    ...
    assert_eq!(unique_samples.len(), sample_count, 
    ...
    for (i, sample) in random_samples.iter().enumerate() {
        assert_ne!(sample, &vec![0u8; 32], "Sample {} should not be all zeros", i);
        assert_ne!(sample, &vec![255u8; 32], "Sample {} should not be all ones", i);
    }
    ...
    for (byte_value, &frequency) in byte_frequencies.iter().enumerate() {
        assert!(frequency <= max_allowed,
                "Byte value {} appears {} times, exceeding threshold {}",
                byte_value, frequency, max_allowed);
    }
    
    // Test 4: Each byte value should appear at least once in large sample
    let min_required = total_bytes / 1000; // Very low threshold
    let missing_values: Vec<usize> = byte_frequencies.iter().enumerate()
        .filter(|(_, &freq)| freq < min_required)
        .map(|(value, _)| value)
        .collect();
    
    assert!(missing_values.len() < 50, 
            "Too many byte values missing or rare: {:?}", missing_values);
}
...
async fn test_commit_reveal_cryptographic_security() {
    ...
    assert_ne!(commitment.commitment, [0u8; 32], "Commitment hash should not be zeros");
    ...
    assert_ne!(reveal.signature.0, [0u8; 64], "Signature should not be zeros");
    ...
    assert!(!commitment.verify_reveal(&wrong_reveal),
    ...
    let is_valid = SecureKeystore::verify_signature(&signature_data, &reveal.signature, &public_key)
...
async fn test_entropy_pool_secure_combination() {
    ...
    assert_ne!(combined1, combined3, "Adding entropy should change combined result");
}
...
async fn test_secure_dice_roll_generation() {
    ...
    for i in 0..6 {
        for j in 0..6 {
            ...
            assert!(count > expected_per_combination - tolerance,
                    "Combination ({},{}) appears too rarely: {} times", i+1, j+1, count);
            assert!(count < expected_per_combination + tolerance,
                    "Combination ({},{}) appears too frequently: {} times", i+1, j+1, count);
        }
    }
    ...
    for i in 0..6 {
        assert!(die1_counts[i] > expected_per_die - die_tolerance,
                "Die1 value {} appears too rarely: {} times", i+1, die1_counts[i]);
        assert!(die1_counts[i] < expected_per_die + die_tolerance,
                "Die1 value {} appears too frequently: {} times", i+1, die1_counts[i]);
        
        assert!(die2_counts[i] > expected_per_die - die_tolerance,
                "Die2 value {} appears too rarely: {} times", i+1, die2_counts[i]);
        assert!(die2_counts[i] < expected_per_die + die_tolerance,
                "Die2 value {} appears too frequently: {} times", i+1, die2_counts[i]);
    }
}
...
async fn test_entropy_generation_uses_os_randomness() {
    ...
    let sequences = (0..100).map(|_| entropy_pool.generate_bytes(32)).collect::<Vec<_>>();
    ...
    for (i, seq) in sequences.iter().enumerate() {
        assert_ne!(seq, &vec![0u8; 32], "Sequence {} should not be all zeros", i);
        assert_ne!(seq, &vec![255u8; 32], "Sequence {} should not be all ones", i);
        ...
        assert!(!has_ascending, "Sequence {} should not be ascending pattern", i);
        assert!(!has_descending, "Sequence {} should not be descending pattern", i);
        assert!(!all_same || seq[0] == 0 || seq[0] == 255, "Sequence {} should not be all same value", i);
    }
    
    // Test different lengths work correctly
    for len in [1, 16, 33, 64, 100] {
        let bytes = entropy_pool.generate_bytes(len);
        assert_eq!(bytes.len(), len, "Generated bytes should have requested length {}", len);
    }
}
...
async fn test_no_dummy_crypto_implementations() {
    ...
    let sig2 = keystore.sign(message).expect("Should create another signature");
    
    assert_ne!(sig1.0, [0u8; 64], "Signature should not be dummy zeros");
    assert_ne!(sig2.0, [0u8; 64], "Second signature should not be dummy zeros");
    
    // Different messages should produce different signatures
    let different_message = b"Different test message";
    let sig3 = keystore.sign(different_message).expect("Should create different signature");
    assert_ne!(sig1.0, sig3.0, "Different messages should produce different signatures");
    ...
    let public_key = keystore.export_public_key();
    assert_eq!(peer_id, public_key, "Peer ID should match exported public key");
    
    // Test 3: Random nonces are truly random
    let nonces: Vec<[u8; 32]> = (0..10).map(|_| keystore.generate_commitment_nonce()).collect();
    ...
    assert_ne!(hash1, hash3, "Different entropy should produce different hash");
}
...
async fn test_concurrent_crypto_operations() {
    ...
    let handles: Vec<_> = (0..10).map(|i| {
        task::spawn(async move {
            ...
            let signature = keystore.sign(message.as_bytes()).expect("Should sign message");
            let nonce = keystore.generate_commitment_nonce();
            let random_bytes = keystore.generate_random_bytes(32);
            let public_key = keystore.export_public_key();
            ...
            assert_ne!(nonce, [0u8; 32], "Nonce should not be zeros in thread {}", i);
            assert_ne!(random_bytes, vec![0u8; 32], "Random bytes should not be zeros in thread {}", i);
    ...
    assert_eq!(random_bytes_set.len(), 10, "All random bytes should be unique");
...
async fn test_crypto_security_under_stress() {
    ...
    for i in 0..iterations {
        ...
        let signature = keystore.sign(message.as_bytes()).expect("Should create signature");
        let nonce = keystore.generate_commitment_nonce();
    ...
    println!("Completed {} crypto operations in {:?}", iterations, duration);
    ...
    assert!(ops_per_second > 100.0, 
            "Should perform at least 100 crypto ops/sec, got {:.2}", ops_per_second);
}