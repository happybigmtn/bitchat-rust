...
use bitcraps::crypto::{SafeArithmetic, token_arithmetic};
...
async fn test_safe_addition_boundaries() {
    ...
    assert_eq!(SafeArithmetic::safe_add_u64(u64::MAX - 1, 1).unwrap(), u64::MAX);
...
async fn test_safe_subtraction_boundaries() {
    ...
    assert_eq!(SafeArithmetic::safe_sub_u64(u64::MAX, 1).unwrap(), u64::MAX - 1);
...
async fn test_safe_division_zero_protection() {
    ...
    assert_eq!(SafeArithmetic::safe_div_u64(u64::MAX, 2).unwrap(), u64::MAX / 2);
...
async fn test_safe_balance_updates() {
    ...
    assert_eq!(SafeArithmetic::safe_balance_update(initial_balance, 500).unwrap(), 1500);
    assert_eq!(SafeArithmetic::safe_balance_update(0, 100).unwrap(), 100);
    
    // Negative balance changes (debits)
    assert_eq!(SafeArithmetic::safe_balance_update(initial_balance, -300).unwrap(), 700);
    assert_eq!(SafeArithmetic::safe_balance_update(1000, -1000).unwrap(), 0);
    
    // Zero change
    assert_eq!(SafeArithmetic::safe_balance_update(initial_balance, 0).unwrap(), initial_balance);
    ...
    assert!(SafeArithmetic::safe_balance_update(large_balance, -(large_balance as i64 + 1)).is_err());
}
...
async fn test_comprehensive_bet_validation() {
    ...
    assert!(SafeArithmetic::safe_validate_bet(player_balance, player_balance, player_balance).is_ok());
    ...
    assert!(SafeArithmetic::safe_validate_bet(player_balance + 1, player_balance, u64::MAX).is_err());
    assert!(SafeArithmetic::safe_validate_bet(player_balance * 2, player_balance, u64::MAX).is_err());
...
async fn test_secure_payout_calculations() {
    ...
    assert_eq!(SafeArithmetic::safe_calculate_payout(bet_amount, 2, 1).unwrap(), 2000); // 2:1 payout
    assert_eq!(SafeArithmetic::safe_calculate_payout(bet_amount, 3, 2).unwrap(), 1500); // 3:2 payout
    assert_eq!(SafeArithmetic::safe_calculate_payout(bet_amount, 1, 1).unwrap(), 1000); // 1:1 payout
    assert_eq!(SafeArithmetic::safe_calculate_payout(bet_amount, 1, 2).unwrap(), 500);  // 1:2 payout
...
async fn test_sequence_increment_protection() {
    ...
    assert_eq!(SafeArithmetic::safe_increment_sequence(u64::MAX - 1).unwrap(), u64::MAX);
    ...
    for i in 0..10 {
        assert_eq!(SafeArithmetic::safe_increment_sequence(near_max + i).unwrap(), near_max + i + 1);
    }
    assert!(SafeArithmetic::safe_increment_sequence(u64::MAX).is_err());
}
...
async fn test_safe_array_access() {
    ...
    assert_eq!(*SafeArithmetic::safe_array_access(&test_array, 0).unwrap(), 10);
    assert_eq!(*SafeArithmetic::safe_array_access(&test_array, 2).unwrap(), 30);
    assert_eq!(*SafeArithmetic::safe_array_access(&test_array, 4).unwrap(), 50);
...
async fn test_comprehensive_error_conditions() {
    ...
    assert!(matches!(overflow_result.unwrap_err(), Error::ArithmeticOverflow(_)));
    
    let div_zero_result = SafeArithmetic::safe_div_u64(100, 0);
    assert!(matches!(div_zero_result.unwrap_err(), Error::DivisionByZero(_)));
    
    let invalid_input_result = SafeArithmetic::safe_percentage(100, 150);
    assert!(matches!(invalid_input_result.unwrap_err(), Error::InvalidInput(_)));
    
    let insufficient_funds_result = SafeArithmetic::safe_validate_bet(1000, 500, 2000);
    assert!(matches!(insufficient_funds_result.unwrap_err(), Error::InsufficientFunds(_)));
    
    let bounds_result = SafeArithmetic::safe_array_access(&[1, 2, 3], 5);
    assert!(matches!(bounds_result.unwrap_err(), Error::IndexOutOfBounds(_)));
