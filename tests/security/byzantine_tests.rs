...
async fn test_byzantine_critical_threshold() {
    ...
    println!("Byzantine threshold test: consensus={}, detected={}", 
...
async fn test_signature_forgery_prevention() {
    ...
    assert!(actions.iter().any(|a| matches!(a, MaliciousAction::SendInvalidSignature)));
...
async fn test_double_spend_prevention() {
    ...
    assert!(actions.iter().any(|a| matches!(a, MaliciousAction::AttemptDoubleSpend)));
    assert!(actions.iter().any(|a| matches!(a, MaliciousAction::ProposeInvalidState)));
}
...
async fn test_byzantine_recovery() {
    ...
    println!("Recovery test: Round 1 consensus={}, Round 2 consensus={}", 
