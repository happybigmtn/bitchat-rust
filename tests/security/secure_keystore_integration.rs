...
async fn test_ed25519_signature_verification() {
    ...
    assert!(
        is_invalid.is_err() || !is_invalid.unwrap(),
...
async fn test_context_based_signatures() {
    ...
    for context in &contexts {
        let sig = keystore
            .sign_with_context(message, context.clone())
    ...
    for (expected_context, signature) in &signatures {
        let is_valid =
            SecureKeystore::verify_secure_signature(message, signature, expected_context)
                .expect("Should verify context signature");
        assert!(
            is_valid,
            "Context signature should verify with correct context"
        );
    }
    ...
    for (wrong_context, signature) in &signatures {
        for test_context in &contexts {
            if std::mem::discriminant(test_context) != std::mem::discriminant(wrong_context) {
                let is_invalid =
                    SecureKeystore::verify_secure_signature(message, signature, test_context)
...
async fn test_signature_timestamp_validation() {
    ...
    let is_invalid =
        SecureKeystore::verify_secure_signature(message, &future_sig, &KeyContext::Consensus)
    ...
    let is_invalid =
        SecureKeystore::verify_secure_signature(message, &old_sig, &KeyContext::Consensus)
...
async fn test_context_key_derivation_uniqueness() {
    ...
    for context in &contexts {
        ...
        assert!(
            public_keys.insert(signature.public_key.clone()),
            "Context {:?} should have unique public key",
            context
        );
    }
...
async fn test_public_key_export_import() {
    ...
    assert!(
        ...
        "Should handle key validation gracefully"
    );
}
