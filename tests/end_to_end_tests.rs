...
use bitcraps::{
    ...
    mesh::{GameSessionManager, MeshPeer, MeshService},
    protocol::craps::CrapsGame,
    protocol::{BetType, DiceRoll, GameId, PeerId},
    session::{SessionLimits, SessionManager},
    token::{Account, ProofOfRelay, TokenLedger},
...
async fn test_complete_game_lifecycle() {
    ...
    ledger
        .create_account(bitcraps::TREASURY_ADDRESS)
        .await
        .unwrap();

    // Fund accounts from treasury for testing
    ledger
        .transfer(bitcraps::TREASURY_ADDRESS, creator_id, 10000)
        .await
        .unwrap();
    ledger
        .transfer(bitcraps::TREASURY_ADDRESS, player_id, 5000)
        .await
        .unwrap();
    ...
    // Place bets
    ledger
        .transfer(creator_id, bitcraps::TREASURY_ADDRESS, bet_amount)
        .await
        .unwrap();
    ledger
        .transfer(player_id, bitcraps::TREASURY_ADDRESS, bet_amount)
        .await
        .unwrap();
...
async fn test_token_economy_flow() {
    ...
    ledger
        .transfer(bitcraps::TREASURY_ADDRESS, player1, 1000)
        .await
        .unwrap();
    ledger
        .transfer(bitcraps::TREASURY_ADDRESS, player2, 1500)
        .await
        .unwrap();
...
async fn test_proof_of_relay_incentives() {
    ...
    // Set up accounts
    ledger
        .create_account(Account::new(relay_node, 100))
        .unwrap();
    ...
    assert!(
        relay_reward > 0,
        "Relay nodes should be rewarded for their service"
    );
}
...
async fn test_game_fairness_validation() {
    ...
    assert!(
        game.is_resolved(),
        "Game should be resolved after valid reveal"
    );
}
...
mod performance_tests {
    ...
    async fn test_high_throughput_transactions() {
        ...
        for (i, account) in accounts.iter().enumerate() {
            ledger
                .create_account(Account::new(*account, 1000 + i as u64))
                .unwrap();
        }
        ...
        assert!(
            duration.as_secs() < 5,
            "High-throughput transactions should be fast"
        );
    }
}
...
mod integration_scenarios {
    ...
    async fn test_complete_casino_session() {
        ...
        ledger
            .create_account(Account::new(casino_house, 1_000_000))
            .unwrap(); // House bankroll
        ...
        session_manager
            .join_session(&game_session, player2)
            .await
            .unwrap();
        ...
        println!(
            "Final balances - Player1: {}, Player2: {}, House: {}",
