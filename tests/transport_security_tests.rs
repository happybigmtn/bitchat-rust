...
use bitcraps::transport::{
    crypto::{TransportCrypto, ConnectionPriority, SecureKeyExchange},
    bounded_queue::{BoundedEventQueue, QueueConfig, OverflowBehavior, BoundedQueueError},
    secure_gatt_server::{SecureGattServer, GattClient},
    TransportEvent,
};
...
async fn test_ecdh_key_exchange() {
    ...
    assert!(crypto1.perform_key_exchange(peer_id2, public2).await.is_ok());
    assert!(crypto2.perform_key_exchange(peer_id1, public1).await.is_ok());
    ...
    let decrypted1 = crypto2.decrypt_message(peer_id1, &encrypted1).await.unwrap();
    
    assert_eq!(message, &decrypted1[..]);
    
    let encrypted2 = crypto2.encrypt_message(peer_id1, message).await.unwrap();
    let decrypted2 = crypto1.decrypt_message(peer_id2, &encrypted2).await.unwrap();
...
async fn test_encryption_message_sizes() {
    ...
    crypto1.perform_key_exchange(peer_id2, public2).await.unwrap();
    crypto2.perform_key_exchange(peer_id1, public1).await.unwrap();
    ...
    for size in test_sizes {
        ...
        assert_ne!(message, encrypted[18..].to_vec(), "Ciphertext equals plaintext for size {}", size);
    }
}
...
async fn test_connection_prioritization() {
    ...
    crypto.perform_key_exchange(peer1, public_key).await.unwrap();
    crypto.perform_key_exchange(peer2, public_key).await.unwrap();
    crypto.perform_key_exchange(peer3, public_key).await.unwrap();
    
    // Set different priorities
    crypto.set_connection_priority(peer1, ConnectionPriority::Low).await.unwrap();
    crypto.set_connection_priority(peer2, ConnectionPriority::High).await.unwrap();
    crypto.set_connection_priority(peer3, ConnectionPriority::Critical).await.unwrap();
    
    // Update metrics to generate scores
    crypto.update_connection_metrics(peer1, 100, true).await.unwrap();
    crypto.update_connection_metrics(peer2, 50, true).await.unwrap();
    crypto.update_connection_metrics(peer3, 25, true).await.unwrap();
...
async fn test_connection_metrics() {
    ...
    // Mock key exchange
    let public_key = crypto.public_key();
    crypto.perform_key_exchange(peer_id, public_key).await.unwrap();
    
    // Test successful metrics
    crypto.update_connection_metrics(peer_id, 50, true).await.unwrap();
    crypto.update_connection_metrics(peer_id, 60, true).await.unwrap();
    
    // Test failed metrics
    crypto.update_connection_metrics(peer_id, 200, false).await.unwrap();
...
async fn test_bounded_queue_overflow() {
    ...
    assert!(sender.send(1).await.is_ok());
    assert!(matches!(sender.send(2).await, Err(BoundedQueueError::QueueFull)));
}
...
async fn test_gatt_server_security() {
    ...
    assert!(gatt_server.start().await.is_ok());
    assert!(gatt_server.handle_client_connected(peer_id, client_address).await.is_ok());
    ...
    assert!(gatt_server.disconnect_client(peer_id, "Test disconnect".to_string()).await.is_ok());
...
async fn test_gatt_server_prioritization() {
    ...
    // Connect clients
    assert!(gatt_server.handle_client_connected(peer1, "addr1".to_string()).await.is_ok());
    assert!(gatt_server.handle_client_connected(peer2, "addr2".to_string()).await.is_ok());
    assert!(gatt_server.handle_client_connected(peer3, "addr3".to_string()).await.is_ok());
    
    // Set different priorities
    assert!(gatt_server.set_client_priority(peer1, ConnectionPriority::Low).await.is_ok());
    assert!(gatt_server.set_client_priority(peer2, ConnectionPriority::High).await.is_ok());
    assert!(gatt_server.set_client_priority(peer3, ConnectionPriority::Critical).await.is_ok());
...
async fn test_key_rotation() {
    ...
    let decrypted2 = crypto2.decrypt_message(peer_id1, &encrypted2).await.unwrap();
...
async fn test_transport_security_integration() {
    ...
    // Set priorities
    crypto1.set_connection_priority(peer_id2, ConnectionPriority::High).await.unwrap();
    crypto2.set_connection_priority(peer_id1, ConnectionPriority::High).await.unwrap();
    ...
    for (i, &message) in messages.iter().enumerate() {
        ...
        // Update metrics
        crypto1.update_connection_metrics(peer_id2, 10 + i as u32, true).await.unwrap();
        crypto2.update_connection_metrics(peer_id1, 10 + i as u32, true).await.unwrap();
...
async fn test_encryption_performance() {
    ...
    for size in message_sizes {
        ...
        if size <= 1024 {
            assert!(duration < Duration::from_millis(10), "Encryption too slow for size {}: {:?}", size, duration);
        }
    }
}