...
use bitcraps::database::{DatabasePool, Database, Migration};
use bitcraps::protocol::{PeerId, GameId, CrapTokens};
use std::sync::Arc;
...
async fn test_player_persistence() {
    ...
    pool.create_player(player_id, initial_balance).await.unwrap();
    ...
    pool.update_player_balance(player_id, new_balance).await.unwrap();
...
async fn test_data_integrity() {
    ...
    pool.create_player(player_id, CrapTokens::from_raw(1000)).await.unwrap();
    pool.create_game_session(game_id, player_id, vec![player_id]).await.unwrap();
...
async fn test_database_statistics() {
    ...
    for i in 0..10 {
        let player_id = PeerId::from_bytes(&i.to_le_bytes());
        pool.create_player(player_id, CrapTokens::from_raw(100 * i as u64)).await.unwrap();
    }
