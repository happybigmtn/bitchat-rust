...
async fn test_transaction_handling() {
    ...
    let result = pool
        .transaction(|conn| {
            ...
            Err(bitcraps::error::Error::InvalidData(
    ...
    let table_exists = pool
        .with_connection(|conn| {
            let mut stmt = conn
                .prepare(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='transaction_test'",
                )
    ...
    assert!(
        !table_exists || {
            let count: i64 = pool
                .with_connection(|conn| {
                    let mut stmt = conn
                        .prepare("SELECT COUNT(*) FROM transaction_test")
                        .map_err(|e| bitcraps::error::Error::InvalidData(e.to_string()))?;
                    stmt.query_row([], |row| row.get(0))
                        .map_err(|e| bitcraps::error::Error::InvalidData(e.to_string()))
                })
