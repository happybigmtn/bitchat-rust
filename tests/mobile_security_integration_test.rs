...
use bitcraps::mobile::{
    Argon2Config, BiometricAuthManager, BiometricType, HkdfAlgorithm, KeyDerivationManager,
    MobileSecurityConfig, MobileSecurityManager, PbkdfAlgorithm, Permission, PermissionManager,
    PermissionState, SecureStorageManager, SecurityLevel, SecurityPolicy,
};
...
async fn test_biometric_authentication() -> Result<()> {
    ...
    let auth_result = biometric_manager
        .authenticate_user("Test mobile security integration")
        .await;
    match auth_result {
        Ok(session) => {
            println!(
...
async fn test_key_derivation() -> Result<()> {
    ...
    assert_ne!(
        key_hierarchy.encryption_key.as_bytes(),
        key_hierarchy.signing_key.as_bytes()
    );
    assert_ne!(
        key_hierarchy.encryption_key.as_bytes(),
        key_hierarchy.authentication_key.as_bytes()
    );
    assert_ne!(
        key_hierarchy.signing_key.as_bytes(),
        key_hierarchy.session_key.as_bytes()
    );
...
async fn test_permission_management() -> Result<()> {
    ...
    let camera_result = permission_manager
        .request_permission(
            Permission::Camera,
    ...
    let batch_results = permission_manager
        .request_permissions(
            batch_permissions,
    ...
    let bluetooth_explanation =
        permission_manager.get_permission_explanation(Permission::Bluetooth);
    println!(
...
async fn test_mobile_security_integration() -> Result<()> {
    ...
    let security_manager = match MobileSecurityManager::new(config).await {
        ...
        Err(e) => {
            println!(
                "Mobile security manager creation failed (expected in test environment): {}",
                e
            );
    ...
    if status.can_create_wallets {
        ...
        let secure_wallet = security_manager
            .create_secure_wallet(wallet_id, None)
            .await?;
    ...
    let encrypted = security_manager
        .encrypt_sensitive_data(test_data, context)
        .await?;
    println!(
...
async fn test_cross_platform_compatibility() -> Result<()> {
    ...
    println!(
        ...
        storage_manager.is_ok()
    );
    assert!(storage_manager.is_ok());

    let biometric_manager = BiometricAuthManager::new();
    println!(
        "BiometricAuthManager creation: {:?}",
        biometric_manager.is_ok()
    );
    ...
    let permission_manager =
        PermissionManager::new(vec![Permission::Bluetooth], vec![Permission::Camera]);
    ...
    let summary = permission_manager.check_all_permissions()?;
    println!(
        "Permission check completed, can continue: {}",
