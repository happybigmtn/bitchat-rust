...
use bitcraps::{
    ...
    protocol::{random_peer_id, BetType, PeerId},
    token::TokenLedger,
};
...
async fn test_crypto_operations() {
    ...
    assert_ne!(
        identity1.peer_id, identity2.peer_id,
        "Different keypairs should produce different peer IDs"
    );
    ...
    assert!(
        identity1.keypair.verify(message, &signature),
        "Signature should verify with correct key"
    );
    assert!(
        !identity2.keypair.verify(message, &signature),
...
mod performance_tests {
    ...
    async fn test_operation_performance() {
        ...
        assert!(
            duration.as_secs() < 30,
            "Operations should complete within 30 seconds"
        );
