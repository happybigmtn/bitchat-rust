...
async fn test_house_edge_simulation() {
    ...
    assert!(
        house_edge > 0.5 && house_edge < 2.5,
        "House edge of {:.2}% is outside expected range",
        house_edge
    );
...
async fn test_betting_strategies() {
    ...
    for (strategy, profit) in &strategy_results {
        ...
        assert!(
            *profit < 0,
            "Strategy {} should not be profitable long-term",
            strategy
        );
    }
}
...
fn test_token_economics() {
    ...
    assert!(
        ...
        "Total supply should decrease with higher burn rate"
    );
