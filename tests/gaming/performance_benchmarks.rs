...
fn test_game_creation_performance() {
    ...
    assert!(
        ...
        "Game creation took {}ms, expected < 100ms",
        elapsed.as_millis()
    );
}
...
fn test_bet_processing_performance() {
    ...
    assert!(
        elapsed.as_secs() < 1,
        "Bet processing took {}s, expected < 1s",
        elapsed.as_secs()
    );
}
...
fn test_payout_calculation_performance() {
    ...
    assert!(
        ...
        "Payout calculation took {}ms, expected < 10ms",
        elapsed.as_millis()
    );
}
...
fn test_concurrent_game_operations() {
    ...
    assert!(
        elapsed.as_millis() < 100,
        "Concurrent operations took {}ms, expected < 100ms",
        elapsed.as_millis()
    );
}
