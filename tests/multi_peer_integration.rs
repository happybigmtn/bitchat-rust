...
use bitcraps::{
    ...
    protocol::{GameId, BetType, CrapTokens},
};
...
async fn test_consensus_betting() {
    ...
    peer1.place_bet(game_id, BetType::Pass, CrapTokens(100)).await.unwrap();
    peer2.place_bet(game_id, BetType::DontPass, CrapTokens(100)).await.unwrap();
...
async fn test_partition_recovery() {
    ...
    peer1.place_bet(game_id, BetType::Pass, CrapTokens(50)).await.unwrap();
...
async fn test_byzantine_tolerance() {
    ...
    // Create game
    let game_id = honest_peers[0].create_game(3, CrapTokens(100)).await.unwrap();
    ...
    for peer in &honest_peers {
        peer.place_bet(game_id, BetType::Pass, CrapTokens(50)).await.unwrap();
    }
