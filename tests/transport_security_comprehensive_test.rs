...
use bitcraps::transport::security::{
    EnhancedTransportSecurity, BleSecurityConfig, EncryptedIdentityStorage,
};
use bitcraps::transport::keystore::{
    SecureTransportKeystore, KeyType, KeystoreConfig,
};
...
async fn test_authenticated_key_exchange_with_identity() {
    ...
    assert!(security1.perform_authenticated_key_exchange(
        peer_id2, public2, Some(identity2.clone()), None
    ).await.is_ok());
    
    assert!(security2.perform_authenticated_key_exchange(
        peer_id1, public1, Some(identity1.clone()), None
    ).await.is_ok());
}
...
async fn test_message_fragmentation() {
    ...
    let encrypted_fragments = security1.encrypt_and_authenticate(
        peer_id2, &large_message, 3
    ).await.unwrap();
    
    assert!(encrypted_fragments.len() > 1, "Message should be fragmented");
    ...
    for fragment in encrypted_fragments {
        if let Some(result) = security2.decrypt_and_verify(peer_id1, &fragment).await.unwrap() {
...
async fn test_hmac_authentication() {
    ...
    // Verify HMAC passes
    let decrypted = security2.decrypt_and_verify(peer_id1, ciphertext).await.unwrap();
...
async fn test_timestamp_validation() {
    ...
    let result = security2.decrypt_and_verify(peer_id1, &encrypted_fragments[0]).await;
    assert!(result.is_ok());
...
async fn test_key_rotation() {
    ...
    // Setup initial keys
    security1.perform_authenticated_key_exchange(
        peer_id, public_key, None, None
    ).await.unwrap();
    
    // Encrypt with initial keys
    let message1 = b"Message before rotation";
    let encrypted1 = security1.encrypt_and_authenticate(
        peer_id, message1, 7
    ).await.unwrap();
    ...
    let encrypted2 = security1.encrypt_and_authenticate(
        peer_id, message2, 8
    ).await.unwrap();
...
async fn test_keystore_backup_restore() {
    ...
    keystore.create_backup(&backup_path, "backup_password_123").await.unwrap();
    ...
    // Restore from backup
    keystore.restore_backup(&backup_path, "backup_password_123").await.unwrap();
...
fn test_encrypted_identity_storage() {
    ...
    assert_ne!(encrypted.encrypted_private_key, identity.keypair.secret_key_bytes());
    ...
    assert!(storage.decrypt_identity(&encrypted, wrong_password).is_err());
}
...
async fn test_security_statistics() {
    ...
    security1.perform_authenticated_key_exchange(
        peer_id2, public2, None, Some(aes_config)
    ).await.unwrap();
    
    security2.perform_authenticated_key_exchange(
        peer_id1, public1, None, Some(chacha_config)
    ).await.unwrap();
...
async fn test_comprehensive_security_integration() {
    ...
    let keystore1 = SecureTransportKeystore::new(temp_dir1.path()).await.unwrap();
    let keystore2 = SecureTransportKeystore::new(temp_dir2.path()).await.unwrap();
    
    keystore1.initialize("password1").await.unwrap();
    keystore2.initialize("password2").await.unwrap();
    
    // Store identities
    keystore1.store_identity(&identity1, "my_identity").await.unwrap();
    keystore2.store_identity(&identity2, "my_identity").await.unwrap();
    ...
    security1.perform_authenticated_key_exchange(
        identity2.peer_id, public2, Some(identity2.clone()), Some(config.clone())
    ).await.unwrap();
    
    security2.perform_authenticated_key_exchange(
        identity1.peer_id, public1, Some(identity1.clone()), Some(config)
    ).await.unwrap();
    ...
    // Encrypt messages
    let encrypted1 = security1.encrypt_and_authenticate(
        identity2.peer_id, message1to2, 10
    ).await.unwrap();
    
    let encrypted2 = security2.encrypt_and_authenticate(
        identity1.peer_id, message2to1, 11
    ).await.unwrap();
    
    // Decrypt and verify
    let decrypted1 = security2.decrypt_and_verify(
        identity1.peer_id, &encrypted1[0]
    ).await.unwrap();
    
    let decrypted2 = security1.decrypt_and_verify(
        identity2.peer_id, &encrypted2[0]
    ).await.unwrap();
    ...
    let encrypted_post = security1.encrypt_and_authenticate(
        identity2.peer_id, post_rotation_message, 12
    ).await.unwrap();
    
    let decrypted_post = security2.decrypt_and_verify(
        identity1.peer_id, &encrypted_post[0]
    ).await.unwrap();
...
async fn test_encryption_performance() {
    ...
    for i in 0..iterations {
        let encrypted = security1.encrypt_and_authenticate(
            peer_id2, &test_message, i
        ).await.unwrap();
        
        let decrypted = security2.decrypt_and_verify(
            peer_id1, &encrypted[0]
        ).await.unwrap();
    ...
    println!("Average latency: {:.2}ms", duration.as_millis() as f64 / iterations as f64);
    
    // Should handle at least 50 operations per second on reasonable hardware
    assert!(ops_per_sec > 50.0, "Encryption performance too slow: {} ops/sec", ops_per_sec);
}