...
use bitcraps::{
    BitCrapsApp,
    protocol::{PeerId, GameId, DiceRoll, BetType, CrapTokens},
...
async fn test_security_module_integration() {
    ...
    let signature = keystore.sign_with_context(
        message,
        bitcraps::crypto::secure_keystore::KeyContext::Consensus
    ).unwrap();
    
    let valid = keystore.verify_signature(
...
async fn test_ble_peripheral_advertising() {
    ...
    if cfg!(not(any(target_os = "android", target_os = "ios", target_os = "linux"))) {
...
async fn test_p2p_message_flow() {
    use bitcraps::protocol::p2p_messages::{P2PMessage, MessageType};
    ...
    // Test compression
    use bitcraps::mobile::compression::{CompressionManager, CompressionAlgorithm};
...
async fn test_network_partition_recovery() {
    ...
    // Create game with all nodes
    let participants: Vec<PeerId> = nodes.iter().map(|n| n.get_peer_id()).collect();
    let game_id = nodes[0].create_consensus_game(participants.clone()).await.unwrap();
...
async fn test_anti_cheat_detection() {
    ...
    let suspicious_rolls = vec![
        DiceRoll { die1: 6, die2: 6, timestamp: 0 },
        DiceRoll { die1: 6, die2: 6, timestamp: 1 },
        DiceRoll { die1: 6, die2: 6, timestamp: 2 },
    ];
