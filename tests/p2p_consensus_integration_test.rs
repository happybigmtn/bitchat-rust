...
use bitcraps::{
    AppConfig, BetType, BitchatIdentity, CrapTokens, Error, GameId, MeshService, PeerId, Result,
    TokenLedger, TransportCoordinator,
};
...
impl TestNode {
    ...
    async fn new() -> Result<Self> {
        ...
        MeshConsensusIntegration::integrate(mesh_service.clone(), consensus_handler.clone())
            .await?;
    ...
    async fn place_bet(&self, game_id: GameId, bet_type: BetType, amount: u64) -> Result<()> {
        self.game_manager
            .place_bet(game_id, bet_type, CrapTokens::new_unchecked(amount))
            .await
    }
...
async fn test_multi_node_game_creation() -> Result<()> {
    ...
    let participants = vec![node1.peer_id(), node2.peer_id(), node3.peer_id()];
...
async fn test_consensus_engine_creation() -> Result<()> {
    ...
    let consensus_engine = Arc::new(tokio::sync::Mutex::new(ConsensusEngine::new(
        game_id,
        participants.clone(),
        identity.peer_id,
...
async fn test_consensus_configuration() -> Result<()> {
    ...
    let engine = ConsensusEngine::new(game_id, participants, identity.peer_id, config)?;
