...
use bitcraps::database::{
    DatabasePool, MigrationManager, Migration,
    UserRepository, GameRepository, TransactionRepository, StatsRepository,
    cache::{DatabaseCache, CacheConfig, WriteStrategy},
    models::*,
    query_builder::{QueryBuilder, UserQueries, GameQueries, ComparisonOperator, SortDirection},
};
...
async fn test_migration_system() {
    ...
    assert!(!report.successful.is_empty(), "Should have applied migrations");
    
    // Check migration status
    let conn = rusqlite::Connection::open(&db_path).unwrap();
    let status = manager.with_connection(conn).status(&rusqlite::Connection::open(&db_path).unwrap()).unwrap();
    assert!(status.is_up_to_date, "Database should be up to date");
    assert!(!status.applied.is_empty(), "Should have applied migrations");
    assert!(status.pending.is_empty(), "Should have no pending migrations");
}
...
async fn test_migration_validation_and_rollback() {
    ...
    // Test validation
    let conn = rusqlite::Connection::open(&db_path).unwrap();
    let validation = manager.with_connection(conn).validate(&rusqlite::Connection::open(&db_path).unwrap()).unwrap();
...
async fn test_database_pool_operations() {
    ...
    let count: i64 = pool.with_connection(|conn| {
        let count = conn.query_row("SELECT COUNT(*) FROM test_table", [], |row| row.get(0))
            .map_err(|e| Error::Database(e.to_string()))?;
        Ok(count)
    }).await.unwrap();
...
async fn test_user_repository() {
    ...
    user_repo.create_user(user_id, username, &public_key).await.unwrap();
...
async fn test_transaction_repository() {
    ...
    // Test status update
    tx_repo.update_transaction_status("tx_001", "confirmed").await.unwrap();
    tx_repo.update_transaction_status("tx_002", "confirmed").await.unwrap();
    
    // Test user transaction history
    let alice_txs = tx_repo.get_user_transactions("user_alice", 10).await.unwrap();
...
async fn test_stats_repository() {
    ...
    stats_repo.update_game_stats("game_123", &game_stats).await.unwrap();
    ...
    stats_repo.update_player_stats("player_alice", &player_stats).await.unwrap();
...
async fn test_cache_layer() {
    ...
    cache.put("key4".to_string(), b"value4".to_vec()).await.unwrap();
    cache.put("key5".to_string(), b"value5".to_vec()).await.unwrap();
...
fn test_database_models() {
    ...
    transaction.confirm(Some(12345), Some("hash123".to_string())).unwrap();
...
async fn test_error_handling() {
    ...
    let result = QueryBuilder::delete()
        .build(); // Missing table name
    assert!(result.is_err());
}