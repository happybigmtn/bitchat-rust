...
use bitcraps::mobile::{
    Argon2Config, HkdfAlgorithm, KeyDerivationManager, PbkdfAlgorithm, Permission,
    PermissionManager, PermissionState, SecureStorageManager,
};
...
async fn test_permission_management_functionality() -> Result<()> {
    ...
    let permission_manager =
        PermissionManager::new(required_permissions.clone(), optional_permissions.clone());
    ...
    let bluetooth_permissions = vec![Permission::Bluetooth, Permission::BluetoothAdmin];
    ...
    let location_explanation =
        permission_manager.get_permission_explanation(Permission::AccessCoarseLocation);
...
async fn test_secure_storage_functionality() -> Result<()> {
    ...
    match delete_result {
        ...
        Err(e) => {
            println!(
                "User data deletion failed (may be expected in test mode): {}",
                e
            );
        }
    }
...
async fn test_cross_platform_compatibility() -> Result<()> {
    ...
    assert!(
        ...
        "SecureStorageManager should be creatable on all platforms"
    );
    ...
    println!(
        "Cross-platform master key generated, length: {} bytes",
        test_key.len()
    );
    ...
    let summary = permission_manager.check_all_permissions()?;
    println!(
        "Cross-platform permission check completed, can continue: {}",
