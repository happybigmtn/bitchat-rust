...
**Feynman Explanation**: Week 5 is about building the "casino floor" - the actual interface
where players interact with our decentralized casino. Think of it like the difference between
the casino's internal machinery (weeks 1-4) and the actual slot machines, craps tables, and
cashier windows that players use. The CLI is like a text-based casino where you type commands
to place bets, and the TUI (Terminal UI) is like having actual tables and displays showing
the game state, your chips, and other players.

Implement the user-facing application layer with CLI interface and terminal UI for BitCraps, including specialized casino UI components for BitCraps gaming and wallet integration.
...
## Day 6: Persistence Layer

### Goals
- Implement SQLite database for game history
- Create key-value store for peer data
- Build wallet persistence with encryption
...
cipher = "0.4"  # For wallet encryption
```
...
Week 5 delivers a complete application layer with CLI, TUI, and persistence:

- **CLI Framework**: Full command-line interface with clap
- **Terminal UI**: Rich interactive interface with ratatui
- **Casino Interface**: Complete betting and gaming UI
- **Wallet Management**: Token balance tracking and transactions
- **Persistence Layer**: SQLite for history, Sled for key-value storage
- **Encrypted Storage**: Secure wallet persistence with password protection
- **Session Recovery**: Restore state after disconnection
- **Game History**: Complete audit trail of all games and bets
