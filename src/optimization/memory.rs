...
impl MmapStorage {
    pub fn new(file_path: String, capacity_mb: usize) -> Result<Self, std::io::Error> {
        ...
        if capacity_mb > MAX_CAPACITY_MB {
            return Err(std::io::Error::new(
                ...
                format!(
                    "Capacity {}MB exceeds maximum {}MB",
        ...
        if capacity_mb < MIN_CAPACITY_MB {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Capacity {}MB below minimum {}MB",
                    capacity_mb, MIN_CAPACITY_MB
                ),
            ));
        }
        ...
        let capacity = capacity_mb
            ...
            .ok_or_else(|| {
                std::io::Error::new(std::io::ErrorKind::InvalidInput, "Capacity overflow")
            })?;
        ...
        if metadata.len() != capacity as u64 {
            return Err(std::io::Error::other("Failed to set file size correctly"));
        }
        ...
        let mmap = unsafe {
            ...
            MmapOptions::new().len(capacity).map_mut(&file)?
        };
    ...
    pub fn store(&mut self, key: &[u8], data: &[u8]) -> Result<bool, std::io::Error> {
        ...
        let new_used = self.used.checked_add(required_space).ok_or_else(|| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
        ...
        let mmap = match self.mmap.as_mut() {
            Some(mmap) => mmap,
            None => return Err(std::io::Error::other("Memory map not initialized")),
        };
