...
use crate::protocol::craps::{BetType, DiceRoll, CrapTokens};
...
pub enum OrchestratorEvent {
    GameAdvertised { game_id: GameId, advertisement: GameAdvertisement },
    GameDiscovered { game_id: GameId, advertisement: GameAdvertisement },
    PlayerJoinRequested { game_id: GameId, player: PeerId },
    PlayerJoined { game_id: GameId, player: PeerId },
    PlayerLeft { game_id: GameId, player: PeerId, reason: String },
    TurnChanged { game_id: GameId, current_player: Option<PeerId>, previous_player: Option<PeerId> },
    TurnTimeout { game_id: GameId, player: PeerId },
    BetValidated { game_id: GameId, bet: BetRecord },
    DiceCommitted { game_id: GameId, round_id: u64, roller: PeerId },
    DiceRevealed { game_id: GameId, round_id: u64, roll: DiceRoll },
    PayoutCalculated { game_id: GameId, payouts: HashMap<PeerId, CrapTokens> },
    StateConflictDetected { game_id: GameId, conflicting_peers: Vec<PeerId> },
}

/// Commands for orchestrator control
#[derive(Debug)]
pub enum OrchestratorCommand {
    AdvertiseGame { config: GameConfig, max_players: usize },
    DiscoverGames { filters: GameDiscoveryRequest },
    JoinGame { game_id: GameId, initial_balance: CrapTokens },
    LeaveGame { game_id: GameId },
    PlaceBet { game_id: GameId, bet_type: BetType, amount: CrapTokens },
    CommitDiceRoll { game_id: GameId, commitment_hash: [u8; 32] },
    RevealDiceRoll { game_id: GameId, dice_roll: DiceRoll, nonce: [u8; 32] },
    ForceStateSync { game_id: GameId },
    ResolveConflict { game_id: GameId, resolution_data: Vec<u8> },
}
...
impl GameOrchestrator {
    ...
    pub fn new(
        ...
        consensus_manager: Arc<ConsensusGameManager>,
        config: OrchestratorConfig,
    ) -> (Self, mpsc::UnboundedSender<OrchestratorCommand>, broadcast::Receiver<OrchestratorEvent>) {
    ...
    pub async fn advertise_game(&self, config: GameConfig) -> Result<GameId> {
        ...
        // Store advertisement
        self.advertised_games.write().await.insert(game_id, advertisement.clone());
        ...
        log::info!("Advertised game {:?} of type '{}'", game_id, config.game_type);
        Ok(game_id)
    }
    ...
    pub async fn process_join_request(&self, request: GameJoinRequest) -> Result<GameJoinResponse> {
        ...
        if let Some(session) = sessions.get(&game_id) {
            ...
            if let Some(ad) = advertisement {
                ...
                let state_snapshot = sessions.get(&game_id)
                    .map(|s| s.state_snapshot.clone())
                    .unwrap_or_else(|| Arc::new(RwLock::new(GameStateSnapshot {
                        current_phase: "waiting".to_string(),
                        active_bets: Vec::new(),
                        game_history: Vec::new(),
                        current_turn: None,
                ...
                let participants = sessions.get(&game_id)
    ...
    fn create_advertisement_packet(&self, advertisement: &GameAdvertisement) -> Result<Vec<u8>> {
        let serialized = bincode::serialize(advertisement)
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (game_id, ad) in games.iter() {
                    if now > ad.created_at + 300 { // 5 minute TTL
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (game_id, timed_out_player) in timeouts {
                    ...
                    log::warn!("Turn timeout for player {:?} in game {:?}", timed_out_player, game_id);
                }
            }
        });
                ...
                for (game_id, session) in sessions.iter() {
                    ...
                    if let Some(consensus_session) = consensus_manager.get_game_state(game_id).await {
                        ...
                        if let Some(started) = turn_manager.turn_started {
                            ...
                            if let Some(deadline_system) = now_system.checked_sub(now_instant.saturating_duration_since(deadline)) {
                                state.turn_deadline = Some(
                                    deadline_system.duration_since(UNIX_EPOCH)
                                        .unwrap_or_default()
                                        .as_secs()
                                );
                            } else if let Some(deadline_system) = now_system.checked_add(deadline.saturating_duration_since(now_instant)) {
                                state.turn_deadline = Some(
                                    deadline_system.duration_since(UNIX_EPOCH)
                                        .unwrap_or_default()
                                        .as_secs()
            ...
            loop {
                ...
                for (game_id, session) in sessions.iter() {
                    ...
                    if let Some(consensus_session) = consensus_manager.get_game_state(game_id).await {
                        ...
                        let participants_match = session.participants.len() == consensus_session.participants.len();
    ...
    async fn process_command(&self, command: OrchestratorCommand) -> Result<()> {
        match command {
            OrchestratorCommand::AdvertiseGame { config, max_players } => {
            ...
            OrchestratorCommand::JoinGame { game_id, initial_balance } => {
            ...
            OrchestratorCommand::PlaceBet { game_id, bet_type, amount } => {
                self.process_bet_placement(game_id, bet_type, amount).await?;
            }
            
            OrchestratorCommand::CommitDiceRoll { game_id, commitment_hash } => {
                self.commit_dice_roll(game_id, commitment_hash).await?;
            }
            
            OrchestratorCommand::RevealDiceRoll { game_id, dice_roll, nonce } => {
            ...
            OrchestratorCommand::ResolveConflict { game_id, resolution_data } => {
                self.resolve_state_conflict(game_id, resolution_data).await?;
            }
        }
    ...
    async fn process_bet_placement(&self, game_id: GameId, bet_type: BetType, amount: CrapTokens) -> Result<()> {
        // Validate bet through consensus
        self.consensus_manager.place_bet(game_id, bet_type.clone(), amount).await?;
        ...
        if let Some(session) = self.active_sessions.read().await.get(&game_id) {
            ...
            *session.last_activity.write().await = Instant::now();
            ...
            let _ = self.event_tx.send(OrchestratorEvent::BetValidated { game_id, bet: bet_record });
    ...
    async fn commit_dice_roll(&self, game_id: GameId, commitment_hash: [u8; 32]) -> Result<()> {
        ...
        if let Some(session) = self.active_sessions.read().await.get(&game_id) {
            ...
            session.pending_commits.write().await
                .entry(round_id)
    ...
    async fn reveal_dice_roll(&self, game_id: GameId, dice_roll: DiceRoll, nonce: [u8; 32]) -> Result<()> {
        let round_id = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
        ...
        if let Some(session) = self.active_sessions.read().await.get(&game_id) {
            ...
            session.pending_reveals.write().await
                .entry(round_id)
                .or_insert_with(Vec::new)
                .push(reveal);
    ...
    async fn resolve_state_conflict(&self, _game_id: GameId, _resolution_data: Vec<u8>) -> Result<()> {
        // Resolve consensus conflict
        Ok(())
    }
    
    /// Helper method to create discovery packet
    fn create_discovery_packet(identity: &BitchatIdentity, request: &GameDiscoveryRequest) -> Result<Vec<u8>> {
        let serialized = bincode::serialize(request)
            .map_err(|e| Error::Serialization(format!("Failed to serialize discovery request: {}", e)))?;
    ...
    pub async fn get_discovered_games(&self) -> Vec<GameAdvertisement> {
        self.discovered_games.read().await.values().cloned().collect()
    }
