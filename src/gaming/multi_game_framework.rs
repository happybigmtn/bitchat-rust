...
impl MultiGameFramework {
    ...
    pub async fn create_session(&self, request: CreateSessionRequest) -> Result<String, GameFrameworkError> {
        ...
        info!("Created game session: {} for game: {}", session_id, request.game_id);
    ...
    pub async fn join_session(&self, session_id: &str, player_id: String, join_data: PlayerJoinData) -> Result<(), GameFrameworkError> {
        // Get session
    ...
    pub async fn process_action(&self, session_id: &str, player_id: &str, action: GameAction) -> Result<GameActionResult, GameFrameworkError> {
        ...
        *session.last_activity.write().await = SystemTime::now();
        ...
        debug!("Processed action {:?} for player {} in session {}", action, player_id, session_id);
    ...
    /// End game session
    pub async fn end_session(&self, session_id: &str, reason: SessionEndReason) -> Result<SessionSummary, GameFrameworkError> {
        ...
        let summary = SessionSummary {
            ...
            duration: SystemTime::now().duration_since(session.created_at).unwrap_or(Duration::from_secs(0)),
    ...
    fn register_builtin_games(&mut self) {
        ...
        tokio::spawn({
            ...
            async move {
                ...
                if let Err(e) = framework.register_game("poker".to_string(), poker_engine).await {
...
impl GameEngine for CrapsGameEngine {
    ...
    async fn validate_session_config(&self, _config: &GameSessionConfig) -> Result<(), GameEngineError> {
    ...
    async fn validate_player_join(&self, session: &GameSession, _player_id: &str, _join_data: &PlayerJoinData) -> Result<(), GameFrameworkError> {
    ...
    async fn on_player_joined(&self, _session: &GameSession, player_id: &str) -> Result<(), GameFrameworkError> {
        info!("Player {} joined Craps game", player_id);
        Ok(())
    }

    async fn process_action(&self, _session: &GameSession, player_id: &str, action: GameAction) -> Result<GameActionResult, GameFrameworkError> {
        match action {
...
impl GameEngine for BlackjackGameEngine {
    ...
    async fn process_action(&self, _session: &GameSession, player_id: &str, action: GameAction) -> Result<GameActionResult, GameFrameworkError> {
        match action {
            ...
            _ => Err(GameFrameworkError::UnsupportedAction("Action not supported for Blackjack".to_string())),
        }
    }
...
impl GameEngine for PokerGameEngine {
    ...
    async fn process_action(&self, _session: &GameSession, player_id: &str, action: GameAction) -> Result<GameActionResult, GameFrameworkError> {
        match action {
            ...
            _ => Err(GameFrameworkError::UnsupportedAction("Action not supported for Poker".to_string())),
        }
    }
...
pub enum GameActionResult {
    BetPlaced { bet_id: String, confirmation: String },
    DiceRolled { dice: (u8, u8), total: u8 },
    CardDealt { card: u8 },
    PlayerStands,
    PlayerFolds,
    PlayerChecks,
    BetAccepted { new_balance: u64 },
}
...
pub enum GameFrameworkEvent {
    ...
    ActionProcessed { session_id: String, player_id: String, action: GameAction, result: GameActionResult },
}
...
mod tests {
    ...
    async fn test_player_join() {
        ...
        framework.join_session(&session_id, "player1".to_string(), join_data).await.unwrap();
        ...
        framework.join_session(&session_id, "player2".to_string(), join_data2).await.unwrap();
    }
    ...
    async fn test_game_action() {
        ...
        let result = framework.process_action(&session_id, "player1", action).await.unwrap();
        
        match result {
            GameActionResult::BetPlaced { bet_id, confirmation } => {
