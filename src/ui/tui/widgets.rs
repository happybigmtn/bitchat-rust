...
use crate::protocol::{BetType, DiceRoll};
use crate::PeerId;
...
impl BettingTableWidget {
    ...
    fn render_bet_area(
        ...
        let availability_text = if !is_available { " (N/A)" } else { "" };
        ...
        let content = vec![
            ...
            Line::from(""),
            Line::from(vec![Span::styled(odds, text_style)]),
            Line::from(vec![Span::styled(
                availability_text,
                Style::default().fg(Color::Gray),
            )]),
...
impl PlayerStatsWidget {
    ...
    fn render_current_bets(&self, f: &mut Frame, area: Rect) {
        if self.current_bets.is_empty() {
            ...
            let bet_items: Vec<ListItem> = self
                .current_bets
                .iter()
                .map(|(bet_type, amount)| {
