...
impl SecureKeystore {
    ...
    pub async fn init_master_key(&self, password: &str) -> Result<()> {
        ...
        *self.master_key.write().await = Some(master);
        ...
        self.audit("init_master_key", "", true, "Master key initialized")
            .await;
    ...
    /// Generate a new key
    pub async fn generate_key(&self, key_type: KeyType, purpose: &str) -> Result<String> {
        let key_id = format!(
            "{}_{}",
            match key_type {
        ...
        let (raw_bytes, algorithm) = match key_type {
            ...
            KeyType::Master => {
                return Err(Error::Crypto(
                    "Cannot generate master key directly".to_string(),
                ));
            }
        };
        ...
        self.audit(
            ...
            &key_id,
            true,
            &format!("Generated {} key", key_type.as_str()),
        )
        .await;
    ...
    pub async fn rotate_key(&self, key_id: &str) -> Result<String> {
        ...
        let new_key_id = self
            .generate_key(old_key.key_type.clone(), &old_key.metadata.purpose)
            .await?;
    ...
    pub async fn check_rotation_needed(&self) -> Vec<String> {
        ...
        for (key_id, encrypted) in keys.iter() {
            if let Some(expires_at) = encrypted.metadata.expires_at {
                let warning_time =
                    expires_at - self.rotation_policy.rotation_warning_period.as_secs();
                if now >= warning_time {
