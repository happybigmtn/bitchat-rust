...
use prometheus::{
    ...
    Counter, Gauge, Histogram, GaugeVec, CounterVec, HistogramVec, register_counter, register_gauge, register_histogram,
...
impl PrometheusServer {
    ...
    pub async fn start(mut self) -> Result<(), Error> {
        ...
        log::info!("Starting Prometheus metrics server on {}", self.config.bind_address);
...
async fn collect_application_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Network metrics
    NETWORK_MESSAGES_SENT.inc_by(metrics.network.messages_sent.load(std::sync::atomic::Ordering::Relaxed) as f64);
    NETWORK_MESSAGES_RECEIVED.inc_by(metrics.network.messages_received.load(std::sync::atomic::Ordering::Relaxed) as f64);
    NETWORK_BYTES_SENT.inc_by(metrics.network.bytes_sent.load(std::sync::atomic::Ordering::Relaxed) as f64);
    NETWORK_BYTES_RECEIVED.inc_by(metrics.network.bytes_received.load(std::sync::atomic::Ordering::Relaxed) as f64);
    NETWORK_ACTIVE_CONNECTIONS.set(metrics.network.active_connections.load(std::sync::atomic::Ordering::Relaxed) as f64);
    NETWORK_CONNECTION_ERRORS.inc_by(metrics.network.connection_errors.load(std::sync::atomic::Ordering::Relaxed) as f64);
    
    // Consensus metrics
    CONSENSUS_PROPOSALS_SUBMITTED.inc_by(metrics.consensus.proposals_submitted.load(std::sync::atomic::Ordering::Relaxed) as f64);
    CONSENSUS_PROPOSALS_ACCEPTED.inc_by(metrics.consensus.proposals_accepted.load(std::sync::atomic::Ordering::Relaxed) as f64);
    CONSENSUS_PROPOSALS_REJECTED.inc_by(metrics.consensus.proposals_rejected.load(std::sync::atomic::Ordering::Relaxed) as f64);
    CONSENSUS_FORKS.inc_by(metrics.consensus.fork_count.load(std::sync::atomic::Ordering::Relaxed) as f64);
    
    // Error metrics by category
    ERRORS_TOTAL.with_label_values(&["network", "error"]).inc_by(metrics.errors.network_errors.load(std::sync::atomic::Ordering::Relaxed) as f64);
    ERRORS_TOTAL.with_label_values(&["consensus", "error"]).inc_by(metrics.errors.consensus_errors.load(std::sync::atomic::Ordering::Relaxed) as f64);
    ERRORS_TOTAL.with_label_values(&["gaming", "error"]).inc_by(metrics.errors.gaming_errors.load(std::sync::atomic::Ordering::Relaxed) as f64);
    ERRORS_TOTAL.with_label_values(&["system", "critical"]).inc_by(metrics.errors.critical_errors.load(std::sync::atomic::Ordering::Relaxed) as f64);
...
async fn collect_business_metrics() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Gaming metrics
    GAMES_PLAYED.with_label_values(&["craps"]).inc_by(metrics.gaming.total_games.load(std::sync::atomic::Ordering::Relaxed) as f64);
    GAMES_ACTIVE.with_label_values(&["craps"]).set(metrics.gaming.active_games.load(std::sync::atomic::Ordering::Relaxed) as f64);
    BETS_PLACED.with_label_values(&["pass_line"]).inc_by((metrics.gaming.total_bets.load(std::sync::atomic::Ordering::Relaxed) / 3) as f64);
    BETS_PLACED.with_label_values(&["dont_pass"]).inc_by((metrics.gaming.total_bets.load(std::sync::atomic::Ordering::Relaxed) / 4) as f64);
    BETS_PLACED.with_label_values(&["field"]).inc_by((metrics.gaming.total_bets.load(std::sync::atomic::Ordering::Relaxed) / 5) as f64);
    
    BETTING_VOLUME.inc_by(metrics.gaming.total_volume.load(std::sync::atomic::Ordering::Relaxed) as f64);
    PAYOUTS.inc_by(metrics.gaming.total_payouts.load(std::sync::atomic::Ordering::Relaxed) as f64);
    DICE_ROLLS.inc_by(metrics.gaming.dice_rolls.load(std::sync::atomic::Ordering::Relaxed) as f64);
    GAME_DISPUTES.inc_by(metrics.gaming.disputes.load(std::sync::atomic::Ordering::Relaxed) as f64);
