...
impl MacOSSystemMonitor {
    ...
    fn get_cpu_usage_sysctl(&self) -> Result<f32, SystemMonitorError> {
        ...
        let output = Command::new("top")
            ...
            .output()
            .map_err(|e| {
                SystemMonitorError::SystemApiError(format!("Failed to run top command: {}", e))
            })?;
    ...
    fn get_cpu_usage(&self) -> Result<f32, SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "macOS CPU monitoring not available".to_string(),
        ))
    }
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        ...
        let output = Command::new("vm_stat").output().map_err(|e| {
            SystemMonitorError::SystemApiError(format!("Failed to run vm_stat: {}", e))
        })?;
    ...
    fn extract_page_count(&self, line: &str) -> Result<u64, SystemMonitorError> {
        ...
        if let Some(colon_pos) = line.find(':') {
            ...
            Err(SystemMonitorError::ParseError(format!(
                "Invalid vm_stat line: {}",
                line
            )))
        }
    }
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "macOS memory monitoring not available".to_string(),
        ))
    }
    ...
    fn get_battery_info(&self) -> Result<(Option<f32>, Option<bool>), SystemMonitorError> {
        ...
        let output = Command::new("pmset")
            ...
            .output()
            .map_err(|e| {
                SystemMonitorError::SystemApiError(format!("Failed to run pmset: {}", e))
            })?;
                ...
                if let Some(percent_start) = line.find('\t') {
                    ...
                    if let Some(percent_end) = after_tab.find('%') {
                        ...
                        if let Ok(battery_level) = percent_str.parse::<f32>() {
                            let is_charging =
                                line.contains("charging") || line.contains("AC attached");
    ...
    fn get_network_interfaces(
        ...
        let output = Command::new("netstat")
            ...
            .output()
            .map_err(|e| {
                SystemMonitorError::SystemApiError(format!("Failed to run netstat: {}", e))
            })?;
...
impl SystemMonitor for MacOSSystemMonitor {
    fn collect_metrics(&self) -> Result<SystemMetrics, SystemMonitorError> {
        ...
        let (total_memory, used_memory, available_memory) =
            self.get_memory_info().unwrap_or((0, 0, 0));
        let (battery_level, battery_charging) = self.get_battery_info().unwrap_or((None, None));
        let (temperature, thermal_throttling) = self.get_thermal_info().unwrap_or((None, false));
        let thread_count = self.get_thread_count().unwrap_or(1);
        let network_interfaces = self.get_network_interfaces().unwrap_or_default();
...
mod tests {
    ...
    async fn test_collect_metrics() {
        ...
        match monitor.collect_metrics() {
            ...
            Err(e) => {
                println!(
                    "macOS metrics collection failed (expected on non-macOS): {}",
                    e
                );
            }
        }
