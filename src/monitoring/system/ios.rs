...
impl IOSSystemMonitor {
    ...
    fn get_cpu_usage(&self) -> Result<f32, SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "iOS CPU monitoring not available".to_string(),
        ))
    }
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        unsafe {
            ...
            let mut count = std::mem::size_of::<VmStatistics>() as c_uint
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "iOS memory monitoring not available".to_string(),
        ))
    }
