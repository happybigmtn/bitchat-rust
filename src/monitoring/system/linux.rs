...
impl LinuxSystemMonitor {
    ...
    fn read_cpu_stats(&self) -> Result<CpuStats, SystemMonitorError> {
        let content = fs::read_to_string("/proc/stat")
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read /proc/stat: {}", e)))?;
        
        let first_line = content.lines().next()
            .ok_or_else(|| SystemMonitorError::ParseError("Empty /proc/stat".to_string()))?;
        
        if !first_line.starts_with("cpu ") {
            return Err(SystemMonitorError::ParseError("Invalid /proc/stat format".to_string()));
        }
        
        let parts: Vec<&str> = first_line.split_whitespace().collect();
        if parts.len() < 11 {
            return Err(SystemMonitorError::ParseError("Insufficient CPU stats in /proc/stat".to_string()));
        }
        
        let parse_u64 = |s: &str| s.parse::<u64>()
            .map_err(|e| SystemMonitorError::ParseError(format!("Failed to parse CPU stat '{}': {}", s, e)));
    ...
    fn read_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        let content = fs::read_to_string("/proc/meminfo")
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read /proc/meminfo: {}", e)))?;
        ...
        for line in content.lines() {
            if let Some(colon_pos) = line.find(':') {
                ...
                let value = value_str.trim().parse::<u64>()
                    .map_err(|e| SystemMonitorError::ParseError(format!("Failed to parse memory value '{}': {}", value_str, e)))?;
    ...
    fn read_battery_info(&self) -> Result<(Option<f32>, Option<bool>), SystemMonitorError> {
        ...
        let entries = fs::read_dir(power_supply_path)
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read power supply dir: {}", e)))?;
    ...
    fn read_battery_capacity(&self, battery_path: &std::path::Path) -> Result<Option<f32>, SystemMonitorError> {
        let capacity_path = battery_path.join("capacity");
        match fs::read_to_string(&capacity_path) {
            Ok(content) => {
                let capacity = content.trim().parse::<f32>()
                    .map_err(|e| SystemMonitorError::ParseError(format!("Failed to parse battery capacity: {}", e)))?;
                Ok(Some(capacity))
            },
            Err(_) => Ok(None),
        }
    ...
    fn read_battery_charging_status(&self, battery_path: &std::path::Path) -> Result<Option<bool>, SystemMonitorError> {
    ...
    fn read_thermal_info(&self) -> Result<(Option<f32>, bool), SystemMonitorError> {
        ...
        let entries = fs::read_dir(thermal_path)
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read thermal dir: {}", e)))?;
            ...
            if name.starts_with("thermal_zone") {
                ...
                if let Ok(temp_str) = fs::read_to_string(&temp_path) {
                    if let Ok(temp_millicelsius) = temp_str.trim().parse::<i32>() {
                        ...
                        max_temp = Some(max_temp.map_or(temp_celsius, |existing| existing.max(temp_celsius)));
                    }
                }
        ...
        match fs::read_dir(task_dir) {
            ...
            Err(e) => {
                ...
                let status = fs::read_to_string("/proc/self/status")
                    .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read process status: {}", e)))?;
                
                for line in status.lines() {
                    if line.starts_with("Threads:") {
                        let count_str = line.split_whitespace().nth(1)
                            .ok_or_else(|| SystemMonitorError::ParseError("Invalid Threads line in /proc/self/status".to_string()))?;
                        let count = count_str.parse::<u32>()
                            .map_err(|e| SystemMonitorError::ParseError(format!("Failed to parse thread count: {}", e)))?;
                        return Ok(count);
                    }
                }
                
                Err(SystemMonitorError::SystemApiError(format!("Failed to determine thread count: {}", e)))
            }
        }
    }
    
    fn read_network_interfaces(&self) -> Result<HashMap<String, NetworkInterface>, SystemMonitorError> {
        let mut interfaces = HashMap::new();
        
        // Read from /proc/net/dev
        let content = fs::read_to_string("/proc/net/dev")
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to read /proc/net/dev: {}", e)))?;
        ...
        for line in content.lines().skip(2) { // Skip header lines
            ...
            if parts.len() >= 17 {
                ...
                let parse_u64 = |s: &str| s.parse::<u64>()
                    .map_err(|e| SystemMonitorError::ParseError(format!("Failed to parse network stat '{}': {}", s, e)));
