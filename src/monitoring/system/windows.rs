...
impl WindowsSystemMonitor {
    ...
    fn get_cpu_usage(&self) -> Result<f32, SystemMonitorError> {
        unsafe {
            ...
            if result != 0 {
                return Err(SystemMonitorError::SystemApiError(format!(
                    "PdhOpenQuery failed: {}",
                    result
                )));
            }
            ...
            // Add CPU counter
            let counter_path: Vec<u16> = "\\Processor(_Total)\\% Processor Time\0"
                .encode_utf16()
                .collect();
    ...
    fn get_cpu_usage(&self) -> Result<f32, SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "Windows CPU monitoring not available".to_string(),
        ))
    }
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        unsafe {
            ...
            if result == 0 {
                return Err(SystemMonitorError::SystemApiError(
    ...
    fn get_memory_info(&self) -> Result<(u64, u64, u64), SystemMonitorError> {
        Err(SystemMonitorError::PlatformNotSupported(
            "Windows memory monitoring not available".to_string(),
        ))
    }
...
mod tests {
    ...
    async fn test_collect_metrics() {
        ...
        match monitor.collect_metrics() {
            ...
            Err(e) => {
                println!(
                    "Windows metrics collection failed (expected on non-Windows): {}",
                    e
                );
            }
        }
