...
use super::{SystemMonitor, SystemMetrics, SystemMonitorError, MetricType};
use super::linux::LinuxSystemMonitor;

#[cfg(target_os = "android")]
use jni::{JNIEnv, JavaVM, objects::{JClass, JObject, JString, JValue}, sys::jint};
...
impl AndroidSystemMonitor {
    ...
    #[cfg(target_os = "android")]
    fn get_battery_info_with_env(&self, env: &mut JNIEnv) -> Result<(Option<f32>, Option<bool>), SystemMonitorError> {
        ...
        let context_class = env.find_class("android/content/Context")
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to find Context class: {:?}", e)))?;
        
        // Get BatteryManager system service
        let battery_service = env.get_static_field(
            context_class, 
            "BATTERY_SERVICE", 
            "Ljava/lang/String;"
        ).map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to get BATTERY_SERVICE: {:?}", e)))?;
        ...
        log::warn!("Android JNI battery monitoring requires proper context setup - falling back to Linux");
        self.linux_monitor.read_battery_info()
    }
    ...
    #[cfg(target_os = "android")]
    fn get_thermal_info_with_env(&self, env: &mut JNIEnv) -> Result<(Option<f32>, bool), SystemMonitorError> {
        // Access Android's PowerManager.ThermalService
        // This requires Android API level 29+
        
        // Get PowerManager class
        let power_manager_class = env.find_class("android/os/PowerManager")
            .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to find PowerManager class: {:?}", e)))?;
            ...
            match jvm.attach_current_thread() {
                Ok(mut env) => {
                    ...
                    let activity_manager_class = env.find_class("android/app/ActivityManager")
                        .map_err(|e| SystemMonitorError::SystemApiError(format!("Failed to find ActivityManager: {:?}", e)))?;
...
pub mod jni_setup {
    ...
    fn initialize_android_context(env: &mut JNIEnv, context: JObject) -> Result<(), SystemMonitorError> {
...
mod tests {
    ...
    async fn test_collect_metrics() {
        ...
        match monitor.collect_metrics() {
            Ok(metrics) => {
                println!("Android Metrics - CPU: {}%, Memory: {} MB", 
