...
impl FallbackSystemMonitor {
    ...
    fn generate_simulated_metrics(&self) -> Result<SystemMetrics, SystemMonitorError> {
        ...
        let cpu_usage = (state.cpu_base + cpu_variation * state.trend).max(0.0).min(100.0);
        state.cpu_base = (state.cpu_base * 0.98 + cpu_usage * 0.02).max(2.0).min(80.0); // Smooth trending
        
        // Generate memory usage
        let total_memory = 8 * 1024 * 1024 * 1024u64; // 8GB total
        let memory_variation = (fastrand::f32() - 0.5) * 0.1; // ±5%
        let used_memory = ((state.memory_base as f32) * (1.0 + memory_variation * state.trend)) as u64;
        ...
        let temperature = if fastrand::f32() < 0.7 { // 70% chance
        ...
        for (i, &name) in interface_names.iter().enumerate() {
            ...
            network_interfaces.insert(name.to_string(), NetworkInterface {
                name: name.to_string(),
                is_up: is_active,
...
mod tests {
    ...
    async fn test_collect_simulated_metrics() {
        ...
        println!("Metrics 1 - CPU: {}%, Memory: {} MB", 
                 metrics1.cpu_usage_percent, 
                 metrics1.used_memory_bytes / 1024 / 1024);
        println!("Metrics 2 - CPU: {}%, Memory: {} MB", 
    ...
    fn test_realistic_simulation() {
        ...
        assert!(memory_values.iter().all(|&mem| mem > 0 && mem <= 8 * 1024 * 1024 * 1024));
        
        // Values should vary (not all identical)
        let cpu_variance = cpu_values.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() - 
                          cpu_values.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        assert!(cpu_variance > 0.0, "CPU values should vary");
        
        println!("CPU range: {:.1}% - {:.1}%", 
                 cpu_values.iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
                 cpu_values.iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap());
    }
}