...
pub fn compute_merkle_root<T: AsRef<[u8]>>(items: &[T]) -> [u8; 32] {
    let data: Vec<Vec<u8>> = items.iter().map(|item| item.as_ref().to_vec()).collect();
...
mod tests {
    ...
    fn test_randomness_combination() {
        let sources = vec![[1u8; 32], [2u8; 32], [3u8; 32]];
    ...
    fn test_pbkdf2_key_derivation() {
        ...
        let result2 = KeyDerivation::derive_key_pbkdf2(
