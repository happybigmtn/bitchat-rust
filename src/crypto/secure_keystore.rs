...
impl SecureKeystore {
    ...
    pub fn verify_signature(
        ...
        let verifying_key = VerifyingKey::from_bytes(public_key)
            .map_err(|_| crate::error::Error::InvalidPublicKey("Invalid public key format".to_string()))?;
    ...
    pub fn verify_secure_signature(
        ...
        let pk_bytes: [u8; 32] = signature.public_key.as_slice().try_into()
            .map_err(|_| crate::error::Error::InvalidPublicKey("Invalid public key length in signature".to_string()))?;
        let verifying_key = VerifyingKey::from_bytes(&pk_bytes)
            .map_err(|_| crate::error::Error::InvalidPublicKey("Invalid public key in signature".to_string()))?;
        
        let sig_bytes: [u8; 64] = signature.signature.as_slice().try_into()
            .map_err(|_| crate::error::Error::InvalidSignature("Invalid signature length".to_string()))?;
    ...
    pub fn verify_peer_public_key(public_key: &[u8; 32]) -> Result<VerifyingKey> {
        VerifyingKey::from_bytes(public_key)
            .map_err(|_| crate::error::Error::InvalidPublicKey("Invalid peer public key".to_string()))
    }
}
...
mod tests {
    ...
    fn test_signature_creation_and_verification() {
        ...
        let is_invalid = SecureKeystore::verify_signature(wrong_message, &signature, &public_key).unwrap();
        assert!(!is_invalid);
    }
    ...
    fn test_context_signing() {
        ...
        let signature = keystore.sign_with_context(message, KeyContext::Consensus).unwrap();
