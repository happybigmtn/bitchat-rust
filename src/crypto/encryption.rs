...
impl Encryption {
    ...
    pub fn decrypt(encrypted: &[u8], private_key: &[u8; 32]) -> Result<Vec<u8>, String> {
        if encrypted.len() < 32 + 12 + 16 {
            // ephemeral_pub + nonce + min_ciphertext
        ...
        let nonce_bytes: [u8; 12] = encrypted[32..44].try_into().map_err(|_| "Invalid nonce")?;
...
mod tests {
    ...
    fn test_encryption_decryption() {
        ...
        println!(
            "Encrypted length: {}, expected at least: {}",
            encrypted.len(),
            32 + 12 + message.len() + 16
        );
