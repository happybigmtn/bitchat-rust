...
impl AdaptiveMTU {
    ...
    pub async fn discover_mtu(&self, peer: PeerId, probe_fn: impl Fn(usize) -> bool) -> Result<usize> {
        ...
        while low <= high && probe_count < 10 {  // Limit probes to avoid excessive testing
    ...
    pub async fn verify_cached_mtu(&self, peer: PeerId, probe_fn: impl Fn(usize) -> bool) -> Result<()> {
        let should_verify = {
            let cache = self.discovered_mtu.read().await;
            if let Some(cached) = cache.get(&peer) {
