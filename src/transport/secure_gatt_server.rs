...
use crate::transport::{
    bounded_queue::{BoundedQueueError, BoundedTransportEventSender},
...
pub const BITCRAPS_KEY_EXCHANGE_CHAR_UUID: Uuid =
    Uuid::from_u128(0x12345678_1234_5678_1234_567812345681);
...
impl SecureGattServer {
        ...
        tokio::spawn({
            ...
            async move {
                services
                    .write()
                    .await
                    .insert(BITCRAPS_SERVICE_UUID, bitcraps_service);
            }
        });
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        log::info!(
            "Starting secure GATT server for peer {:?}",
            self.local_peer_id
        );
    ...
    pub async fn stop(&self) -> Result<()> {
        ...
        *self.is_running.write().await = false;
        ...
        for peer_id in clients {
            let _ = self
                .disconnect_client(peer_id, "Server shutting down".to_string())
                .await;
        }
    ...
    pub async fn handle_client_connected(
            ...
            .await
        {
            ...
            Err(BoundedQueueError::QueueFull) => {
                log::warn!(
                    "Event queue full, dropping connection event for peer {:?}",
                    peer_id
                );
            }
            Err(e) => {
                log::error!(
                    "Failed to send connection event for peer {:?}: {}",
                    peer_id,
                    e
                );
            }
    ...
    pub async fn disconnect_client(&self, peer_id: PeerId, reason: String) -> Result<()> {
            ...
            .await
        {
            ...
            Err(BoundedQueueError::QueueFull) => {
                log::warn!(
                    "Event queue full, dropping disconnection event for peer {:?}",
                    peer_id
                );
            }
            Err(e) => {
                log::error!(
                    "Failed to send disconnection event for peer {:?}: {}",
                    peer_id,
                    e
                );
            }
    ...
    pub async fn handle_characteristic_write(
        ...
        log::debug!(
            "Characteristic write from {:?} to {}: {} bytes",
            peer_id,
            char_uuid,
            data.len()
        );
        ...
        match char_uuid {
            BITCRAPS_RX_CHAR_UUID => self.handle_data_received(peer_id, data).await,
            BITCRAPS_KEY_EXCHANGE_CHAR_UUID => self.handle_key_exchange(peer_id, data).await,
            _ => {
                log::warn!(
                    "Write to unknown characteristic {} from peer {:?}",
                    char_uuid,
                    peer_id
                );
                Ok(())
    ...
    async fn handle_key_exchange(&self, peer_id: PeerId, key_data: Vec<u8>) -> Result<()> {
        if key_data.len() != 32 {
            return Err(Error::Crypto(
                "Invalid key exchange data length".to_string(),
            ));
        }
    ...
    pub async fn send_to_client(&self, peer_id: PeerId, data: &[u8]) -> Result<()> {
        ...
        let client =
            client.ok_or_else(|| Error::Network(format!("Client {:?} not connected", peer_id)))?;
        ...
        if encrypted_data.len() <= max_payload {
            // Single notification
            self.send_notification(peer_id, BITCRAPS_TX_CHAR_UUID, encrypted_data)
                .await?;
        } else {
            // Multiple notifications (fragmented)
            for chunk in encrypted_data.chunks(max_payload) {
                self.send_notification(peer_id, BITCRAPS_TX_CHAR_UUID, chunk.to_vec())
                    .await?;
        ...
        log::debug!(
            "Sent {} encrypted bytes to client {:?}",
            encrypted_data_len,
            peer_id
        );
        Ok(())
    ...
    async fn send_notification(
        ...
        log::debug!(
            "Sending notification to {:?} on characteristic {}: {} bytes",
            peer_id,
            char_uuid,
            data.len()
        );
