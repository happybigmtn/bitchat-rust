...
impl DbusConnection {
    ...
    async fn call_method(
        ...
        log::debug!("D-Bus call: {}.{}.{} on {}", destination, interface, method, path);
    ...
    async fn set_property(
        ...
        log::debug!("D-Bus set property: {}.{} = ? on {}", interface, property, path);
    ...
    async fn register_object(&self, path: &str, interfaces: &[&str]) -> Result<()> {
        log::debug!("Registering D-Bus object at {} with interfaces: {:?}", path, interfaces);
...
impl LinuxBlePeripheral {
    ...
    async fn register_gatt_application(&self, config: &AdvertisingConfig) -> Result<()> {
        ...
        let dbus = self.dbus_connection.as_ref().ok_or_else(|| {
            Error::Network("D-Bus connection not initialized".to_string())
        })?;
    ...
    async fn register_gatt_service(&self, dbus: &DbusConnection, config: &AdvertisingConfig) -> Result<()> {
    ...
    pub async fn send_to_linux_central(&mut self, peer_id: PeerId, data: &[u8]) -> Result<()> {
        ...
        if centrals.contains_key(&peer_id) {
            ...
            characteristics.insert(bluez_constants::BITCRAPS_TX_CHAR_PATH.to_string(), data.to_vec());
    ...
    pub async fn handle_characteristic_write(&self, characteristic_path: &str, value: Vec<u8>, sender: &str) {
        log::debug!("Received characteristic write: {} bytes on {} from {}", value.len(), characteristic_path, sender);
...
pub mod dbus_handlers {
    ...
    pub async fn handle_write_value(
        ...
        peripheral.handle_characteristic_write(&characteristic_path, value, &sender).await;
        Ok(())
    }
    ...
    pub async fn handle_read_value(
        ...
        Ok(characteristics.get(&characteristic_path).cloned().unwrap_or_default())
    }
