...
mod core_bluetooth_ffi {
    ...
    pub type PeripheralManagerCharacteristicCallback = extern "C" fn(*mut c_void, *mut CBCentral, *mut CBMutableCharacteristic, *const u8, usize);
    ...
    extern "C" {
        ...
        pub fn cb_mutable_service_new(service_uuid: *const c_char, is_primary: bool) -> *mut CBMutableService;
...
impl IosBlePeripheral {
    ...
    pub async fn initialize_core_bluetooth(&mut self) -> Result<()> {
        log::info!("Initializing iOS/macOS Core Bluetooth for peer {:?}", self.local_peer_id);
        ...
        if peripheral_manager.is_null() {
            return Err(Error::Network("Failed to create CBPeripheralManager".to_string()));
        }
    ...
    async fn wait_for_powered_on(&self) -> Result<()> {
        ...
        while attempts < MAX_ATTEMPTS {
            ...
            match state {
                ...
                core_bluetooth_ffi::CBManagerState::Unsupported => {
                    return Err(Error::Network("Bluetooth Low Energy is not supported on this device".to_string()));
                }
                core_bluetooth_ffi::CBManagerState::Unauthorized => {
                    return Err(Error::Network("App is not authorized to use Bluetooth Low Energy".to_string()));
                }
                _ => {
        ...
        Err(Error::Network("Timeout waiting for Bluetooth to become ready".to_string()))
    }
    ...
    async fn create_core_bluetooth_service(&mut self) -> Result<()> {
        ...
        let service = unsafe {
            core_bluetooth_ffi::cb_mutable_service_new(service_uuid_cstr.as_ptr(), true)
        };
        
        if service.is_null() {
            return Err(Error::Network("Failed to create CBMutableService".to_string()));
        }
    ...
    async fn create_tx_characteristic(&mut self) -> Result<()> {
        ...
        if tx_characteristic.is_null() {
            return Err(Error::Network("Failed to create TX characteristic".to_string()));
        }
        
        // Add characteristic to service
        if let Some(service) = self.bitcraps_service {
            unsafe {
                core_bluetooth_ffi::cb_mutable_service_add_characteristic(service, tx_characteristic);
            }
        }
    ...
    async fn create_rx_characteristic(&mut self) -> Result<()> {
        ...
        if rx_characteristic.is_null() {
            return Err(Error::Network("Failed to create RX characteristic".to_string()));
        }
        
        // Add characteristic to service
        if let Some(service) = self.bitcraps_service {
            unsafe {
                core_bluetooth_ffi::cb_mutable_service_add_characteristic(service, rx_characteristic);
            }
        }
    ...
    pub async fn start_ios_advertising(&mut self, config: &AdvertisingConfig) -> Result<()> {
        ...
        let peripheral_manager = self.peripheral_manager.ok_or_else(|| {
            Error::Network("Peripheral manager not initialized".to_string())
        })?;
        ...
        let local_name_cstr = if config.include_name {
            Some(std::ffi::CString::new(config.local_name.clone())
                .map_err(|e| Error::Network(format!("Failed to create local name CString: {}", e)))?)
        } else {
            None
        };
        ...
        unsafe {
            core_bluetooth_ffi::cb_peripheral_manager_start_advertising(
                ...
                local_name_cstr.as_ref().map_or(std::ptr::null(), |s| s.as_ptr()),
            );
        }
    ...
    pub async fn send_to_ios_central(&mut self, peer_id: PeerId, data: &[u8]) -> Result<()> {
        ...
        if let Some((central_ptr, _address)) = centrals.get(&peer_id) {
            let tx_characteristic = self.tx_characteristic.ok_or_else(|| {
            ...
            if success {
                ...
                Err(Error::Network(format!("Failed to send data to central {:?}", peer_id)))
            }
        } else {
    ...
    extern "C" fn peripheral_manager_connection_callback(
        ...
        let central_id = unsafe {
            std::ffi::CStr::from_ptr(central_id_cstr).to_string_lossy().into_owned()
        };
        ...
        tokio::spawn(async move {
            ...
            let _ = peripheral.event_sender.send(PeripheralEvent::CentralConnected {
                peer_id,
    ...
    extern "C" fn peripheral_manager_characteristic_callback(
        ...
        let received_data = unsafe {
            std::slice::from_raw_parts(data, data_length).to_vec()
        };
        
        log::debug!("Received {} bytes from central {}", received_data.len(), central_id);
