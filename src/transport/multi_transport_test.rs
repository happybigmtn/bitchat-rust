...
mod tests {
    ...
    use crate::transport::{
        intelligent_coordinator::{
            IntelligentCoordinatorConfig, IntelligentTransportCoordinator, TransportCapabilities,
    ...
    async fn test_concurrent_connections_8_plus() {
        ...
        assert!(
            total_connections >= 4,
            "Should have at least 4 connections established"
        );
        ...
        for (i, peer_id) in peer_ids.iter().enumerate() {
            ...
            match coordinator
                .send_intelligent(*peer_id, test_message.clone(), priority)
                .await
            {
                Ok(_) => {
                    successful_sends += 1;
        ...
        assert!(
            successful_sends > 0,
            "Should have at least some successful message sends"
        );
    }
    ...
    async fn test_nat_traversal_scenarios() {
        ...
        for (scenario_name, nat_type) in nat_scenarios {
            ...
            for dest in test_destinations {
                ...
                println!(
                    "  Destination {}: Selected transport mode: {:?}",
                    dest, selected_mode
                );
                ...
                match nat_type {
                    ...
                    NatType::Symmetric => assert!(matches!(
                        selected_mode,
            ...
            match nat_handler
                .initiate_advanced_nat_traversal(target_peer)
                .await
            {
                Ok(established_addr) => {
                ...
                Err(e) => {
                    println!(
                        "  Advanced traversal failed (expected for some NAT types): {}",
                        e
                    );
    ...
    async fn test_transport_failover() {
        ...
        for i in 0..5 {
            match coordinator
                .send_intelligent(peer_id, test_message.clone(), TransportPriority::Normal)
                .await
            {
    ...
    async fn test_load_balancing() {
        ...
        match coordinator
            .broadcast_optimized(broadcast_message, TransportPriority::Normal)
            .await
        {
            Ok(successful_count) => {
        ...
        for (transport_id, stat) in stats {
            let load_percentage =
                (stat.active_connections as f32 / stat.max_connections as f32) * 100.0;
            println!(
                "Transport {}: {:.1}% loaded ({}/{})",
    ...
    async fn test_performance_stress() {
        ...
        for (transport_id, stat) in final_stats {
            println!(
                "Final stats for {}: {} messages, {} errors, {:.2} reliability",
        ...
        assert!(
            completed_tasks >= 40,
            "Should complete at least 80% of stress test tasks"
        );
        assert!(
