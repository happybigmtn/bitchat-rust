...
impl TransportCryptoState {
    ...
    fn rotate_keys(&mut self, new_shared_secret: &[u8]) -> Result<()> {
        ...
        self.recv_counters.retain(|_, timestamp| *timestamp > cutoff);
...
impl TransportCrypto {
    ...
    pub async fn perform_key_exchange(
        ...
        self.connection_scores.write().await.insert(peer_id, ConnectionScore::default());
    ...
    pub async fn encrypt_message(
        ...
        if state.needs_rotation() {
            ...
            log::warn!("Key rotation needed for peer {:?} but not implemented", peer_id);
        }
        ...
        let mut message = Vec::with_capacity(
            MessageHeader::SIZE + nonce_bytes.len() + ciphertext.len()
        );
    ...
    pub async fn decrypt_message(
        ...
        if state.recv_counters.len() > 10000 {
            let cutoff = Instant::now() - MAX_MESSAGE_AGE;
            state.recv_counters.retain(|_, timestamp| *timestamp > cutoff);
        }
...
mod tests {
    ...
    async fn test_connection_prioritization() {
        ...
        // Mock key exchange
        crypto.perform_key_exchange(peer_id, crypto.public_key()).await.unwrap();
        
        // Set high priority
        crypto.set_connection_priority(peer_id, ConnectionPriority::High).await.unwrap();
