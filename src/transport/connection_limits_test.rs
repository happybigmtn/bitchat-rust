...
mod tests {
    ...
    async fn test_total_connection_limits() {
        ...
        coordinator.connections.write().await.insert(peer1, addr1.clone());
        coordinator.increment_connection_count(&addr1).await;
        
        coordinator.connections.write().await.insert(peer2, addr2.clone());
        ...
        assert!(result.unwrap_err().to_string().contains("Maximum total connections"));
    }
    ...
    async fn test_per_peer_connection_limits() {
        ...
        coordinator.connections.write().await.insert(peer1, addr.clone());
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Maximum connections per peer"));
    }
    ...
    async fn test_connection_cooldown() {
        ...
        let result = coordinator.check_connection_limits(&addr).await;
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Cooldown period active"));
    }
