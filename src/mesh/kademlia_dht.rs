...
impl KademliaDHT {
    ...
    pub fn store_value(&mut self, key: Hash256, value: Vec<u8>) -> Result<(), Error> {
        if value.len() > 65536 {
            return Err(Error::InvalidState("Value too large for DHT storage".into()));
        }
        ...
        log::debug!("Storing value with key {:?} to {} nodes", key, closest.len());
            ...
            for node in &nodes {
                if !lookup.queried.contains(&node.id) && node.id != self.local_id {
                    ...
                    if should_query {
                        ...
                        lookup.closest_nodes.sort_by_key(|n| Distance::between(&n.id, &lookup.target));
    ...
    pub fn is_lookup_complete(&self, lookup_id: &Hash256) -> bool {
        if let Some(lookup) = self.lookups.get(lookup_id) {
            ...
            let closest_queried = lookup.closest_nodes.iter()
                .take(K_BUCKET_SIZE)
    ...
    pub fn cleanup(&mut self) {
        ...
        let completed: Vec<Hash256> = self.lookups.iter()
    ...
    fn update_metrics(&mut self) {
        self.metrics.total_nodes = self.buckets.iter()
            .map(|b| b.nodes.len())
            .sum();
    }
