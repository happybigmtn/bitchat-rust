...
use crate::mobile::{power_management::{PowerManager, ThermalState}, PowerMode};
...
impl MobileProfiler {
                    ...
                    if let Ok(thermal_info) = power_manager.get_thermal_info().await {
                        ...
                        // Update thermal metrics
                        {
                            ...
                            thermal.add_thermal_sample(ThermalSample {
                                ...
                                ambient_temperature: thermal_info.ambient_temperature.unwrap_or(25.0),
    ...
    pub async fn profile_power_consumption<F, R>(&mut self, operation_name: &str, operation: F) -> Result<(R, PowerConsumptionProfile), BitCrapsError>
    where
    ...
    async fn get_current_battery_level(&self) -> Result<f32, crate::error::Error> {
        let battery_info = self.power_manager.get_battery_info().await
            .map_err(|_| crate::error::Error::InvalidData("Failed to get battery info".to_string()))?;
...
impl BatteryMetrics {
    ...
    pub fn average_drain_rate(&self) -> f32 {
        ...
        let drain_samples: Vec<f32> = self.samples.iter()
...
impl ThermalMetrics {
    ...
    pub fn average_battery_temperature(&self) -> f32 {
        ...
        self.samples.iter().map(|s| s.battery_temperature).sum::<f32>() / self.samples.len() as f32
    }
    
    pub fn peak_cpu_temperature(&self) -> f32 {
        self.samples.iter().map(|s| s.cpu_temperature).fold(0.0, f32::max)
    }
    
    pub fn peak_battery_temperature(&self) -> f32 {
        self.samples.iter().map(|s| s.battery_temperature).fold(0.0, f32::max)
    }
    
    pub fn thermal_event_count(&self) -> u32 {
        self.samples.iter()
            .filter(|s| matches!(s.thermal_state, ThermalState::Warning | ThermalState::Critical))
            .count() as u32
    }
    
    pub fn thermal_throttling_duration(&self) -> Duration {
        self.throttling_events.iter()
            .map(|e| e.duration)
            .sum()
    }
}
