...
impl AntiCheatValidator {
    ...
    pub async fn validate_proposal(
        ...
        if violations.is_empty() {
            ...
            let evidence = self.create_cheat_evidence(proposal.proposer, violations.clone()).await;
            Ok(ValidationResult::Suspicious { violations, evidence })
        }
    }
    ...
    pub async fn validate_consensus_message(
        ...
        if violations.is_empty() {
            ...
            let evidence = self.create_cheat_evidence(message.sender, violations.clone()).await;
            Ok(ValidationResult::Suspicious { violations, evidence })
        }
    }
    ...
    async fn validate_bet(
        ...
        if bet.amount.amount() > 1_000_000 { // Arbitrary large amount
        ...
        if let Some(profile) = profiles.get_mut(&player) {
            ...
            if time_since_last < Duration::from_millis(50) { // Too fast
                violations.push(CheatType::TimestampManipulation);
            }
    ...
    async fn validate_dice_roll(
        ...
        if dice_roll.die1 < self.config.min_dice_value || dice_roll.die1 > self.config.max_dice_value {
            violations.push(CheatType::InvalidRoll);
        }
        if dice_roll.die2 < self.config.min_dice_value || dice_roll.die2 > self.config.max_dice_value {
            violations.push(CheatType::InvalidRoll);
        }
    ...
    async fn validate_state_transition(&self, proposal: &GameProposal) -> Result<Vec<CheatType>> {
        ...
        let current_total: u64 = proposal.proposed_state.player_balances.values().map(|b| b.0).sum();
        // In real implementation, would compare against previous state
        
        // Check for impossible state changes
        if proposal.proposed_state.confirmations > 1000 { // Arbitrary sanity check
            violations.push(CheatType::InvalidStateTransition);
        }
    ...
    async fn validate_message_structure(&self, message: &ConsensusMessage) -> Result<Vec<CheatType>> {
        let mut violations = Vec::new();
        ...
        if message.payload_size() > 1_000_000 { // 1MB limit
            violations.push(CheatType::ConsensusViolation);
        }
    ...
    async fn update_randomness_stats(&self, peer_id: PeerId, dice_roll: &DiceRoll) {
        ...
        *stats.dice_outcomes.entry(dice_roll.die2).or_insert(0) += 1;
        ...
        if stats.total_rolls >= 30 { // Minimum sample size
            stats.chi_square_value = self.calculate_chi_square(&stats.dice_outcomes, stats.total_rolls);
        }
        
        // Update global stats
        let mut global_stats = self.global_randomness_stats.write().await;
        *global_stats.dice_outcomes.entry(dice_roll.die1).or_insert(0) += 1;
        *global_stats.dice_outcomes.entry(dice_roll.die2).or_insert(0) += 1;
        ...
        if let Some(stats) = stats_map.get(&peer_id) {
            if stats.total_rolls >= 30 { // Minimum sample size
                ...
                if stats.chi_square_value > critical_value {
                    log::warn!("Statistical anomaly detected for peer {:?}: chi-square = {:.3}", 
                              peer_id, stats.chi_square_value);
                    return Ok(true);
                }
            }
        }
    ...
    async fn create_cheat_evidence(
        ...
        let evidence_id = {
            ...
            hasher.update(&SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs().to_le_bytes());
        ...
        let primary_violation = violations.first().cloned().unwrap_or(CheatType::ConsensusViolation);
        ...
        let evidence = CheatEvidence {
            ...
            detected_at: SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs(),
        ...
        // Store evidence
        self.cheat_evidence.write().await.insert(evidence_id, evidence.clone());
    ...
    pub async fn get_anti_cheat_stats(&self) -> AntiCheatStats {
        ...
        AntiCheatStats {
            ...
            active_investigations: evidence_map.values()
                .filter(|e| e.detected_at >= SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap()
                    .as_secs()
            ...
            statistical_anomalies_detected: profiles.values()
...
mod tests {
    ...
    async fn test_dice_validation() {
        ...
        let violations = validator.validate_dice_roll([2u8; 32], &valid_roll).await.unwrap();
        assert!(violations.is_empty());
        
        // Invalid roll with impossible values
        let invalid_roll = DiceRoll { die1: 7, die2: 8, timestamp: 0 };
        let violations = validator.validate_dice_roll([2u8; 32], &invalid_roll).await.unwrap();
        assert!(!violations.is_empty());
    ...
    async fn test_statistical_analysis() {
        ...
        for _ in 0..50 {
            let biased_roll = DiceRoll { die1: 6, die2: 6, timestamp: 0 };
            validator.update_randomness_stats(biased_peer, &biased_roll).await;
        }
        
        let is_anomalous = validator.detect_statistical_anomaly(biased_peer).await.unwrap();
        assert!(is_anomalous);
    }
}