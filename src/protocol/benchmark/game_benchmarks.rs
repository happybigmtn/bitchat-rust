...
fn bench_state_snapshots(config: &BenchmarkConfig) -> BenchmarkResults {
    ...
    for _ in 0..config.iterations {
        ...
        // Add some deltas
        snapshot.add_delta(
            crate::protocol::efficient_game_state::StateDelta::PhaseChange {
                new_phase: crate::protocol::efficient_game_state::GamePhase::Point,
            },
        );
        snapshot.add_delta(
            crate::protocol::efficient_game_state::StateDelta::RollProcessed {
...
fn bench_batch_bet_resolution(config: &BenchmarkConfig) -> BenchmarkResults {
    ...
    for _ in 0..config.warmup_iterations {
        let _results = resolver
            .resolve_bets_fast(&state, dice_roll, &large_bet_batch[..config.small_dataset])
            .unwrap();
    }
    ...
    for _ in 0..config.iterations {
        ...
        let _results = black_box(
            resolver
                .resolve_bets_fast(&state, dice_roll, &large_bet_batch)
                .unwrap(),
        );
...
fn bench_full_game_simulation(config: &BenchmarkConfig) -> BenchmarkResults {
    ...
    for _ in 0..config.iterations.min(100) {
        // Limit iterations for system tests
        let start = Instant::now();
...
pub fn criterion_game_benchmarks(c: &mut Criterion, _config: &BenchmarkConfig) {
    ...
    c.bench_function("lookup_table_access", |b| {
        let table = PayoutLookupTable::new();
        b.iter(|| black_box(table.lookup_resolution(BetType::Pass, 7)))
    });
}
