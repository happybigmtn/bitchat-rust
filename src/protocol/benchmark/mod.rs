...
impl BitCrapsBenchmarks {
    ...
    fn print_summary(results: &[BenchmarkResults]) {
        ...
        let avg_throughput: f64 = results.iter().map(|r| r.throughput).sum::<f64>() / total_tests as f64;
        let total_memory_bytes: usize = results.iter().map(|r| r.memory_stats.peak_memory_bytes).sum();
        
        println!("Total tests executed: {}", total_tests);
        println!("Average throughput: {:.2} ops/sec", avg_throughput);
        println!("Peak memory usage: {:.2} MB", total_memory_bytes as f64 / 1_048_576.0);
        ...
        for (i, result) in sorted_results.iter().take(5).enumerate() {
            println!("  {}. {} - {:.2} ops/sec", i + 1, result.name, result.throughput);
        }
        
        println!("\nImprovements over baseline:");
        for result in results.iter() {
            if result.improvement_factor > 1.0 {
                println!("  {} - {:.2}x improvement", result.name, result.improvement_factor);
            }
        }
    }
}
