...
use crate::protocol::efficient_consensus::{EfficientDiceConsensus, EntropyAggregator, MerkleTree, ConsensusConfig};
...
pub fn criterion_crypto_benchmarks(c: &mut Criterion, config: &BenchmarkConfig) {
    ...
    c.bench_function("merkle_tree_creation", |b| {
        ...
        b.iter(|| {
            black_box(MerkleTree::new(&leaves).unwrap())
        })
    });
    ...
    c.bench_function("consensus_round", |b| {
        ...
        b.iter(|| {
            let mut consensus = EfficientDiceConsensus::new(game_id, participants.clone(), consensus_config.clone());
            let round_id = 1;
            ...
            for (j, &participant) in participants.iter().enumerate() {
                ...
                consensus.add_commitment(round_id, participant, commitment).unwrap();
                consensus.add_reveal(round_id, participant, nonces[j]).unwrap();
            }
