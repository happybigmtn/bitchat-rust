...
impl StateSynchronizer {
    ...
    pub async fn record_operation(&self, operation: StateOperation) -> Result<()> {
        ...
        if history.len() > self.config.max_history_size {
            let cutoff = operation.sequence.saturating_sub(self.config.max_history_size as u64);
    ...
    pub async fn update_peer_state(&self, peer_id: PeerId, sequence: u64, state_hash: Hash256) {
        ...
        peer_states.insert(peer_id, PeerStateInfo {
            peer_id,
        ...
        self.detect_state_conflicts(sequence, state_hash, peer_id).await;
    ...
    async fn detect_state_conflicts(&self, sequence: u64, state_hash: Hash256, peer_id: PeerId) {
        ...
        if sequence_conflicts.len() > 1 {
            log::warn!("State conflict detected at sequence {}: {} different hashes", 
                      sequence, sequence_conflicts.len());
            
            // Analyze which hash has majority support
            let total_peers = self.peer_states.read().await.len();
            let byzantine_threshold = (total_peers as f64 * self.config.byzantine_threshold) as usize;
            
            for (hash, peers) in sequence_conflicts.iter() {
                if peers.len() >= byzantine_threshold {
                    log::info!("Hash {:?} has majority support ({} peers)", 
                              hex::encode(hash), peers.len());
                } else {
                    log::warn!("Hash {:?} has minority support ({} peers) - potential Byzantine behavior", 
                              hex::encode(hash), peers.len());
                }
            }
        }
    ...
    async fn update_sync_status(&self) {
        ...
        let synchronized_peers = peer_states.values()
            .filter(|info| info.is_synchronized && info.last_updated.elapsed() < Duration::from_secs(60))
            .count();
        
        let total_active_peers = peer_states.values()
        ...
        if *status != new_status {
            log::info!("Sync status changed from {:?} to {:?} (sync ratio: {:.2})", 
                      *status, new_status, sync_ratio);
            *status = new_status;
        }
    }
    ...
    pub async fn request_sync(&self, target_peers: Vec<PeerId>) -> Result<()> {
        ...
        for peer_id in target_peers {
            ...
            log::info!("Requesting state sync from {:?} (current seq: {})", peer_id, current_seq);
    ...
    pub async fn handle_sync_request(&self, requester: PeerId, their_sequence: u64, their_hash: Hash256) -> Result<ConsensusMessage> {
        let current_seq = *self.current_sequence.read().await;
    ...
    pub async fn apply_sync_data(&self, sender: PeerId, compressed_state: CompressedGameState) -> Result<()> {
        // Decompress state data
        let state_bytes = lz4_flex::decompress(&compressed_state.data, compressed_state.original_size as usize)
        ...
        if let Ok(delta) = bincode::deserialize::<StateDelta>(&state_bytes) {
            ...
            return Err(Error::Serialization("Invalid state sync data format".to_string()));
        }
        
        // Update peer state
        self.update_peer_state(sender, compressed_state.sequence, [0u8; 32]).await; // TODO: Get actual hash
    ...
    async fn create_state_delta(&self, from_seq: u64, to_seq: u64) -> Result<StateDelta> {
        ...
        for seq in (from_seq + 1)..=to_seq {
            if let Some(op) = history.get(&seq) {
                ...
                return Err(Error::InvalidData(format!("Missing operation at sequence {}", seq)));
            }
        }
    ...
    async fn apply_state_delta(&self, delta: StateDelta) -> Result<()> {
        ...
        if delta.from_sequence != current_seq {
            return Err(Error::InvalidData("State delta sequence mismatch".to_string()));
        }
    ...
    async fn create_checkpoint(&self, sequence: u64) -> Result<StateCheckpoint> {
        ...
        // Store checkpoint
        self.checkpoints.write().await.insert(sequence, checkpoint.clone());
    ...
    async fn apply_checkpoint(&self, checkpoint: StateCheckpoint) -> Result<()> {
        ...
        let required_signatures = (checkpoint.participants.len() as f64 * self.config.byzantine_threshold) as usize;
        if checkpoint.signatures.len() < required_signatures {
            return Err(Error::InvalidData("Insufficient checkpoint signatures".to_string()));
        }
    ...
    pub async fn detect_partitions(&self) -> Vec<HashSet<PeerId>> {
        ...
        for (peer_id, info) in peer_states.iter() {
            ...
            partition_groups.entry(info.last_known_hash)
                .or_default()
                .insert(*peer_id);
        }
    ...
    pub async fn get_sync_stats(&self) -> SyncStats {
        ...
        let active_conflicts = conflicts.values()
