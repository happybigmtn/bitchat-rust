...
use super::{DiceRoll, GameId, Hash256, PeerId};
...
impl MerkleTree {
    ...
    pub fn new(leaves: &[Hash256]) -> crate::error::Result<Self> {
        ...
        let total_nodes = leaf_count
            ...
            .ok_or_else(|| {
                Error::Protocol("Integer overflow in Merkle tree size calculation".into())
            })?;

        // Additional safety check
        if total_nodes > 100_000_000 {
            return Err(Error::Protocol(format!(
                "Merkle tree too large: {} nodes",
                total_nodes
            )));
        }
...
impl CachedConsensusRound {
    ...
    pub fn add_reveal(&mut self, player: PeerId, nonce: [u8; 32]) -> Result<()> {
        ...
        if !self
            .commitments
            .iter()
            .any(|(p, c)| *p == player && *c == commitment_hash)
        {
            return Err(Error::ValidationError(
                "Invalid reveal - no matching commitment".to_string(),
            ));
        }
...
impl EfficientDiceConsensus {
    ...
    pub fn cleanup_old_rounds(&mut self, max_rounds: usize, timeout_secs: u64) {
        ...
        self.active_rounds
            .retain(|_, round| !round.is_timed_out(timeout_secs));
    ...
    pub fn get_round_status(&self, round_id: u64) -> Option<ConsensusRoundStatus> {
        self.active_rounds
            .get(&round_id)
            .map(|round| ConsensusRoundStatus {
                round_id,
    ...
    pub fn detect_byzantine_behavior(&self, round_id: u64) -> Vec<ByzantineFault> {
        ...
        if let Some(round) = self.active_rounds.get(&round_id) {
            ...
            let commitment_times: Vec<_> = round
                .commitments
                .iter()
                .enumerate()
...
mod tests {
    ...
    fn test_merkle_tree_creation_and_proof() {
        ...
        let proof = tree
            .generate_proof(0)
            .expect("Failed to generate merkle proof in test");
    ...
    fn test_entropy_aggregation() {
        ...
        aggregator
            .add_entropy(&entropy1)
            .expect("Failed to add entropy1 in test");
        aggregator
            .add_entropy(&entropy2)
            .expect("Failed to add entropy2 in test");
    ...
    fn test_dice_roll_generation() {
        ...
        aggregator
            .add_entropy(&[1u8; 32])
            .expect("Failed to add entropy in test");
        aggregator
            .add_entropy(&[255u8; 32])
            .expect("Failed to add entropy in test");
        aggregator
            .add_entropy(&[128u8; 32])
            .expect("Failed to add entropy in test");

        let dice_roll = aggregator
            .generate_dice_roll()
            .expect("Failed to generate dice roll in test");
    ...
    fn test_consensus_round_flow() {
        ...
        round
            .add_commitment(participants[0], commitment1)
            .expect("Failed to add commitment1 in test");
        round
            .add_commitment(participants[1], commitment2)
            .expect("Failed to add commitment2 in test");
        ...
        // Add reveals
        round
            .add_reveal(participants[0], nonce1)
            .expect("Failed to add reveal1 in test");
        round
            .add_reveal(participants[1], nonce2)
            .expect("Failed to add reveal2 in test");
        ...
        let result = round
            .get_result()
            .expect("Failed to get round result in test");
    ...
    fn test_efficient_dice_consensus() {
        ...
        consensus
            ...
            .expect("Failed to start round in test");
        ...
        for (i, &player) in participants.iter().enumerate() {
            ...
            consensus
                .add_commitment(round_id, player, commitment)
                .expect("Failed to add commitment in test");
        }

        // Add reveals
        for (i, &player) in participants.iter().enumerate() {
            consensus
                .add_reveal(round_id, player, nonces[i])
                .expect("Failed to add reveal in test");
        }

        // Process round
        let dice_roll = consensus
            .process_round(round_id)
            .expect("Failed to process round in test");
        assert!(dice_roll.die1 >= 1 && dice_roll.die1 <= 6);
        assert!(dice_roll.die2 >= 1 && dice_roll.die2 <= 6);
    ...
    fn test_byzantine_detection() {
        ...
        consensus
            .start_round(round_id)
            .expect("Failed to start round in byzantine test");
        ...
        consensus
            .add_commitment(round_id, participants[0], commitment)
            .expect("Failed to add commitment for participant 0 in test");
        consensus
            .add_commitment(round_id, participants[1], commitment)
            .expect("Failed to add commitment for participant 1 in test");

        consensus
            .add_reveal(round_id, participants[0], same_nonce)
            .expect("Failed to add reveal for participant 0 in test");
        consensus
            .add_reveal(round_id, participants[1], same_nonce)
            .expect("Failed to add reveal for participant 1 in test");

        // Should detect Byzantine fault
        let faults = consensus.detect_byzantine_behavior(round_id);
        assert!(!faults.is_empty());

        if let ByzantineFault::DuplicateNonce {
            occurrence_count, ..
        } = &faults[0]
        {
    ...
    fn test_merkle_cache_performance() {
        ...
        consensus
            .start_round(round_id)
            .expect("Failed to start round in cache test");
        ...
        consensus
            .add_commitment(round_id, participants[0], commitment)
            .expect("Failed to add commitment in cache test");
