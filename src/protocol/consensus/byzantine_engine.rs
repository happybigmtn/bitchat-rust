...
use crate::protocol::{PeerId, Hash256, Signature, DiceRoll};
...
impl ByzantineConsensusEngine {
    ...
    pub async fn submit_proposal(&self, data: ProposalData) -> Result<Hash256> {
        ...
        proposals.entry(round).or_insert_with(Vec::new).push(proposal);
    ...
    async fn transition_to_voting(&self, round: u64) -> Result<()> {
        ...
        let mut state = self.state.write().await;
        *state = ConsensusState::Voting { round, proposal_hash, deadline };
    ...
    pub async fn submit_vote(&self, proposal_hash: Hash256) -> Result<()> {
        ...
        let (round, expected_hash) = match *state {
            ConsensusState::Voting { round, proposal_hash, deadline } => {
                let now = SystemTime::now()
                    .duration_since(UNIX_EPOCH)
                    .unwrap()
                    .as_secs();
    ...
    pub async fn receive_vote(&self, vote: Vote) -> Result<()> {
        ...
        if !vote.verify(&self.crypto) {
            ...
            self.slash_node(vote.voter, SlashingReason::InvalidVote).await?;
        ...
        let proposal_votes = round_votes.entry(vote.proposal_hash).or_insert_with(Vec::new);
    ...
    async fn finalize_round(&self, round: u64, decision: Hash256) -> Result<()> {
        ...
        let proposal_votes = votes.get(&round)
            .and_then(|rv| rv.get(&decision))
            .ok_or_else(|| Error::Protocol("No votes for decision".into()))?;
        
        let signatures: Vec<Signature> = proposal_votes.iter()
            .map(|v| v.signature)
            .collect();
        
        let participants: Vec<PeerId> = proposal_votes.iter()
            .map(|v| v.voter)
            .collect();
        ...
        let mut state = self.state.write().await;
        *state = ConsensusState::Finalized { round, decision, signatures };
    ...
    pub async fn is_byzantine(&self, node: &PeerId) -> bool {
        ...
        detector.equivocators.contains(node) || 
        detector.invalid_voters.contains(node)
    }
...
mod tests {
    ...
    async fn test_byzantine_detection() {
        ...
        engine.slash_node(byzantine_node, SlashingReason::Equivocation).await.unwrap();
