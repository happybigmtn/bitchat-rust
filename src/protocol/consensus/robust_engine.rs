...
use crate::protocol::{PeerId, Hash256, GameId};
...
impl RobustConsensusEngine {
    ...
    pub fn new(signing_key: SigningKey, treasury: Arc<crate::protocol::treasury::TreasuryManager>) -> Self {
        Self {
            round_id: 0,
    ...
    /// Submit our commit
    pub fn submit_commit(&mut self, value: [u8; 32], nonce: [u8; 32]) -> SignedMessage<RandomnessCommit> {
        ...
        self.commits.insert(self.signing_key.verifying_key().to_bytes(), signed.clone());
        signed
    }
    
    /// Process received commit
    pub fn process_commit(&mut self, signed_commit: SignedMessage<RandomnessCommit>) -> Result<(), Error> {
        // Verify signature
        ...
        // Store commit
        self.commits.insert(signed_commit.content.peer_id, signed_commit);
    ...
    /// Submit our reveal
    pub fn submit_reveal(&mut self, value: [u8; 32], nonce: [u8; 32]) -> Result<SignedMessage<RandomnessReveal>, Error> {
        ...
        self.reveals.insert(self.signing_key.verifying_key().to_bytes(), signed.clone());
        Ok(signed)
    }
    
    /// Process received reveal
    pub fn process_reveal(&mut self, signed_reveal: SignedMessage<RandomnessReveal>) -> Result<(), Error> {
        // Verify signature
        ...
        if let Some(commit) = self.commits.get(&signed_reveal.content.peer_id) {
            ...
            if computed_commitment != commit.content.commitment {
                ...
                *self.penalties.entry(signed_reveal.content.peer_id).or_insert(0.0) += PENALTY_MULTIPLIER;
        ...
        // Store reveal
        self.reveals.insert(signed_reveal.content.peer_id, signed_reveal);
    ...
    pub fn create_proposal(&mut self, game_id: GameId, settlements: Vec<Settlement>) -> Result<SignedMessage<StateProposal>, Error> {
    ...
    /// Vote on a proposal
    pub fn vote_on_proposal(&mut self, proposal_hash: Hash256, approve: bool) -> Result<SignedMessage<ProposalVote>, Error> {
        ...
        self.votes.insert(self.signing_key.verifying_key().to_bytes(), signed.clone());
        ...
        match self.phase {
            ...
            ConsensusPhase::Vote => {
                ...
                if approvals >= threshold {
                    ...
                    self.phase_start = Instant::now();
                } else if self.votes.len() >= self.participants.len() || self.phase_start.elapsed() > SETTLEMENT_TIMEOUT {
    ...
    fn force_settlement(&mut self) {
        ...
        for peer in &self.participants {
            if !self.votes.contains_key(peer) {
                *self.penalties.entry(*peer).or_insert(0.0) += PENALTY_MULTIPLIER * 2.0; // Double penalty for blocking settlement
            }
        }
    }
    ...
    pub fn execute_settlement(&mut self, game_id: GameId) -> Result<Vec<(PeerId, i64)>, Error> {
        ...
        for settlement in &proposal.content.settlements {
            let penalty = self.penalties.get(&settlement.player).copied().unwrap_or(0.0);
