...
use crate::crypto::safe_arithmetic::{token_arithmetic, SafeArithmetic};
...
use super::commit_reveal::{EntropyPool, RandomnessCommit, RandomnessReveal};
use super::validation::{Dispute, DisputeClaim, DisputeEvidence, DisputeVote};
use super::voting::{ConfirmationTracker, Fork, VoteTracker};
use super::{CompactSignature, ConsensusConfig, ConsensusMetrics};
use super::{DisputeId, ProposalId, RoundId, StateHash};
...
impl ConsensusEngine {
    ...
    fn check_byzantine_proposal_consensus(&mut self, proposal_id: ProposalId) -> Result<()> {
        if let Some(vote_tracker) = self.votes.get(&proposal_id) {
            ...
            // CRITICAL FIX: Byzantine fault tolerance requires ceiling of 2n/3 votes
            // Mathematical proof: With n nodes and f Byzantine nodes where f ≤ n/3:
            // - Honest nodes: n - f ≥ n - n/3 = 2n/3
            // - Need majority of honest nodes: > (2n/3)/2 = n/3
            // - But need enough to overcome Byzantine votes: > f + (n-f)/2 = ceiling(2n/3)
            // This ensures safety even when exactly n/3 nodes are Byzantine
    ...
    fn apply_operation_to_state(
        ...
        new_state.sequence_number =
            SafeArithmetic::safe_increment_sequence(new_state.sequence_number)?;
    ...
    fn _apply_operation_to_state_mut(
        &self,
        state: &mut GameConsensusState,
        operation: &GameOperation,
    ) -> Result<()> {
    ...
    fn verify_proposal_signature(&self, proposal: &GameProposal) -> Result<bool> {
        ...
        Ok(crate::crypto::BitchatIdentity::verify_signature(
            &data_to_verify,
            &sig,
        ))
    }
    ...
    fn sign_dispute_vote(&self, dispute_id: DisputeId) -> Result<Signature> {
        ...
        let sig_bytes: [u8; 64] = signature.signature.try_into().map_err(|_| {
            crate::error::Error::InvalidSignature("Failed to convert signature".to_string())
        })?;
    ...
    fn check_dispute_resolution(&mut self, dispute_id: DisputeId) -> Result<()> {
        ...
        if let Some(votes) = votes {
            ...
            let uphold_votes = votes
                .values()
                .filter(|v| matches!(v.vote, super::validation::DisputeVoteType::Uphold))
                .count();
            let dismiss_votes = votes
                .values()
                .filter(|v| matches!(v.vote, super::validation::DisputeVoteType::Reject))
                .count();
    ...
    fn detect_byzantine_voting_patterns(&mut self, proposal_id: ProposalId) -> Result<()> {
        if let Some(vote_tracker) = self.votes.get(&proposal_id) {
            ...
            if total_votes < total_participants / 2 {
                ...
                log::warn!(
                    "Low participation detected for proposal {}: {}/{} votes",
            ...
            if for_ratio > 0.9 || against_ratio > 0.9 {
                ...
                log::warn!(
                    "Suspiciously unanimous voting on proposal {}: {:.2}% for, {:.2}% against",
                    hex::encode(proposal_id),
                    for_ratio * 100.0,
                    against_ratio * 100.0
                );
    ...
    fn verify_state_transition(&self, proposed_state: &GameConsensusState) -> Result<bool> {
        ...
        let expected_sequence =
            SafeArithmetic::safe_increment_sequence(self.current_state.sequence_number)?;
