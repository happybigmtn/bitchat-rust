...
pub enum ConsensusOperation {
    StateUpdate { round_id: u64, state: Vec<u8> },
    VoteReceived { round_id: u64, voter: PeerId, vote: Vec<u8> },
    ProposalSubmitted { round_id: u64, proposal: Vec<u8> },
    DisputeRaised { dispute_id: Hash256, claim: Vec<u8> },
    CheckpointCreated { checkpoint: ConsensusCheckpoint },
}
...
impl ConsensusPersistence {
    ...
    fn get_max_sequence(db: &Connection) -> Result<u64> {
        let result: SqlResult<i64> = db.query_row(
            "SELECT MAX(round_id) FROM consensus_state",
            [],
            |row| row.get(0),
        );
    ...
    pub fn get_votes(&self, round_id: u64) -> Result<Vec<(PeerId, Vec<u8>, [u8; 64])>> {
        ...
        let votes = stmt.query_map(params![round_id as i64], |row| {
...
mod tests {
    ...
    fn test_store_and_load_state() {
        ...
        persistence.store_consensus_state(1, state_hash, state_data).unwrap();
    ...
    fn test_checkpoint_creation() {
        ...
        for i in 1..=CHECKPOINT_INTERVAL {
            ...
            persistence.store_consensus_state(i, hash, state.as_bytes()).unwrap();
        }
