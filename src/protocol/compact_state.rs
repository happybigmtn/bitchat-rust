...
impl CompactGameState {
    ...
    pub fn serialize(&mut self, bets: &[CompactBet], balances: &HashMap<u32, u64>) -> Result<Vec<u8>> {
    ...
    pub fn get_point(&self) -> Option<u8> {
        if self.flags & STATE_FLAG_HAS_POINT != 0 {
            ...
            if point > 0 { Some(point) } else { None }
        } else {
            None
        }
    }
...
impl CompactBet {
    ...
    pub fn deserialize(buf: &mut &[u8]) -> Result<Self> {
        if buf.len() < 7 {
            return Err(Error::InvalidData("Insufficient data for compact bet".to_string()));
        }
...
impl StateMemoryPool {
    ...
    pub fn get_state_buffer(&mut self) -> Vec<u8> {
        self.state_buffers.pop().unwrap_or_else(|| vec![0u8; self.buffer_size])
    }
    ...
    pub fn get_delta_buffer(&mut self) -> Vec<u8> {
        self.delta_buffers.pop().unwrap_or_else(|| vec![0u8; self.buffer_size / 4])
    }
...
impl StateCompressor {
    ...
    pub fn compress_state_sequence(states: &[CompactGameState]) -> Result<(CompactGameState, Vec<StateDelta>)> {
        if states.is_empty() {
    ...
    fn compute_delta(old_state: &CompactGameState, new_state: &CompactGameState, sequence: u16) -> Result<StateDelta> {
    ...
    pub fn reconstruct_state(base: &CompactGameState, deltas: &[StateDelta]) -> Result<CompactGameState> {
