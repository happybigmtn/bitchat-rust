...
impl UltraCompactBet {
    ...
    pub fn from_bet(bet: &super::Bet, game_start_time: u64, _interner: &mut StringInterner) -> Self {
        ...
        let timestamp_delta_secs = (bet.timestamp.saturating_sub(game_start_time) / 1000).min(u16::MAX as u64) as u16;
    ...
    pub fn deserialize(data: &[u8]) -> Result<(Self, usize)> {
        if data.len() < 3 {
            return Err(Error::InvalidData("Insufficient data for ultra compact bet".to_string()));
        }
        ...
        if pos + 2 > data.len() {
            return Err(Error::InvalidData("Insufficient data for timestamp".to_string()));
        }
...
pub trait UltraCompactSerialize {
    ...
    fn ultra_deserialize(data: &[u8]) -> Result<Self> where Self: Sized;
