...
impl NetworkConsensusBridge {
    ...
    pub async fn start(&self) -> Result<()> {
        log::info!("Starting network consensus bridge for game {:?}", self.game_id);
    ...
    pub async fn submit_operation(&self, operation: GameOperation) -> Result<ProposalId> {
        ...
        let proposal_id = self.consensus_coordinator.submit_operation(operation.clone()).await?;
        ...
        self.pending_operations.write().await.insert(proposal_id, pending_op);
    ...
    pub async fn vote_on_proposal(&self, proposal_id: ProposalId, vote: bool) -> Result<()> {
        ...
        self.consensus_coordinator.vote_on_proposal(proposal_id, vote).await?;
                ...
                if let Ok(packet) = Self::message_to_packet(message) {
                    if let Err(e) = mesh_service.broadcast_packet(packet).await {
                        ...
                        *last_state_sync.write().await = Instant::now();
                        log::debug!("Broadcasted state sync for sequence {}", current_state.sequence_number);
                    }
                }
            }
        });
            ...
            loop {
                ...
                operations.retain(|_id, op| {
                    if op.submitted_at.elapsed() > timeout {
                        failed_count += 1;
                        log::warn!("Operation {:?} timed out after {:?}", op.proposal_id, timeout);
                        false
                    } else {
                        true
                    }
                ...
                log::debug!("Cleanup: {} pending operations, {} failed", 
                          operations.len(), failed_count);
            }
        });
    }
    ...
    fn packet_to_message(packet: &BitchatPacket) -> Result<ConsensusMessage> {
        if let Some(payload) = &packet.payload {
            bincode::deserialize(payload)
                .map_err(|e| Error::Serialization(format!("Failed to deserialize consensus message: {}", e)))
        } else {
...
mod tests {
    ...
    async fn test_network_consensus_bridge_creation() {
        ...
        let consensus_engine = Arc::new(Mutex::new(
            ConsensusEngine::new(game_id, participants.clone(), identity.peer_id, ConsensusConfig::default())
