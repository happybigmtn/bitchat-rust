...
impl StateMerkleTree {
    ...
    fn rebuild_tree(&mut self) -> Result<()> {
        ...
        while self.levels[current_level].len() > 1 {
            ...
            while i < self.levels[current_level].len() {
                ...
                let parent_node =
                    self.create_parent_node(left_child, right_child, (current_level + 1) as u8);
    ...
    pub fn memory_usage(&self) -> usize {
        let levels_size = self
            .levels
            .iter()
            .map(|level| level.len() * std::mem::size_of::<MerkleNode>())
            .sum::<usize>();

        let positions_size = self.game_positions.len()
            * (std::mem::size_of::<GameId>() + std::mem::size_of::<usize>());
