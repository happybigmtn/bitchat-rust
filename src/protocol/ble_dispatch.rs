...
impl BleMessageDispatcher {
    ...
    pub async fn queue_message(
        ...
        if message_size >= self.config.compression_threshold {
            ...
            *current_ratio = (*current_ratio * 0.9) + (ratio * 0.1); // Exponential moving average
            ...
            log::debug!(
                "Compressed message from {} to {} bytes (ratio: {:.2})",
                message_size,
                compressed_size,
                ratio
            );
    ...
    pub async fn handle_fragment(
        ...
        if fragment.checksum != expected_checksum {
            log::warn!(
                "Fragment checksum mismatch for message {:?}, fragment {}",
                fragment.message_id,
    ...
    async fn start_dispatch_task(&self) {
        ...
        tokio::spawn(async move {
            let mut dispatch_interval =
                interval(Duration::from_millis(config.connection_interval_ms as u64));

            loop {
                dispatch_interval.tick().await;

                // Calculate available bandwidth for this interval
                let interval_bandwidth =
                    (config.bandwidth_bps as u64 * config.connection_interval_ms as u64) / 1000;
        ...
        while let Some(mut pending) = queue_guard.pop_front() {
            ...
            if message_bytes.len() > config.mtu_size {
                if pending.fragments.is_none() {
                    pending.fragments = Some(Self::fragment_message(
                        message_bytes.clone(),
                        config.mtu_size,
                    ));
                }
    ...
    fn reassemble_message(&self, assembly: &FragmentAssembly) -> Result<ConsensusMessage> {
        ...
        bincode::deserialize(&message_bytes).map_err(|e| Error::Serialization(e.to_string()))
    }
    ...
    pub async fn update_peer_quality(&self, peer_id: PeerId, success: bool, rtt: Duration) {
        ...
        if let Some(peer_stats) = bandwidth_map.get_mut(&peer_id) {
            ...
            peer_stats.connection_quality =
                (peer_stats.connection_quality * 0.9) + (new_quality * 0.1);

            // Update packet loss rate
            let loss_update = if success { 0.0 } else { 1.0 };
            peer_stats.packet_loss_rate =
                (peer_stats.packet_loss_rate * 0.95) + (loss_update * 0.05);
        }
    }
    ...
    pub async fn get_bandwidth_utilization(&self) -> f64 {
        ...
        let available_bytes =
            (self.config.bandwidth_bps as f64 * window_duration.as_secs_f64()) as u64;
