...
impl BleOptimizer {
    ...
    pub async fn update_connection_quality(
        ...
        self.update_connection_priority(peer_id, rssi, latency, success_rate)
            .await;
        ...
        match power_state {
            ...
            PowerState::LowPower => {
                ...
                if message.payload.priority() == MessagePriority::Low {
                    return Err(Error::Network(
                        "Message dropped due to low power mode".to_string(),
                    ));
                }
                message.compress()?;
            }
            PowerState::Sleep => {
                ...
                if message.payload.priority() != MessagePriority::Critical {
                    return Err(Error::Network(
                        "Message dropped due to sleep mode".to_string(),
                    ));
                }
                message.compress()?;
            }
    ...
    async fn start_power_management_task(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let new_state =
                    if battery < config.battery_level_threshold && config.low_power_mode_enabled {
                        PowerState::LowPower
            ...
            loop {
                    ...
                    if avg_reliability > 0.98 && avg_latency < config.latency_target {
                        ...
                        if *interval < config.max_connection_interval {
                            *interval = (*interval + Duration::from_millis(10))
                                .min(config.max_connection_interval);
                            adjustments_made = true;
                        }
                        ...
                        if *interval > config.min_connection_interval {
                            *interval = (*interval - Duration::from_millis(10))
                                .max(config.min_connection_interval);
                            adjustments_made = true;
                        }
                    }
                }
                ...
                if adjustments_made {
                    *adaptive_adjustments.write().await += 1;
                    log::debug!(
                        "Adaptive optimization: MTU={}, Interval={:?}",
                        *current_mtu.read().await,
                        *current_interval.read().await
                    );
                }
            }
        });
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if bandwidth > 80.0 || queues.values().any(|&size| size > 100) {
                    log::warn!(
                        "Performance alert: Bandwidth={}%, Queue sizes={:?}",
                        bandwidth,
                        *queues
                    );
                }
...
mod tests {
    ...
    async fn test_connection_prioritization() {
        ...
        optimizer
            .update_connection_quality(peer1, -30, Duration::from_millis(20), 0.99)
            .await;
        optimizer
            .update_connection_quality(peer2, -80, Duration::from_millis(100), 0.85)
            .await;
