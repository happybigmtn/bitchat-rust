...
use super::{BetType, CrapTokens, DiceRoll, PeerId};
...
static RESOLUTION_CACHE: Lazy<RwLock<ResolutionCache>> =
    Lazy::new(|| RwLock::new(ResolutionCache::new()));
...
impl PayoutLookupTable {
    ...
    fn populate_hardway_bets(&mut self) {
        ...
        for (bet_type, target, payout) in hardway_bets {
            ...
            self.special_requirements
                .insert(bet_type, SpecialRequirement::RequiresHistory(1));
        }
    }
    ...
    fn populate_come_bets(&mut self) {
        ...
        self.special_requirements
            .insert(BetType::Come, SpecialRequirement::RequiresPhase(0b010)); // Point only
        self.special_requirements
            .insert(BetType::DontCome, SpecialRequirement::RequiresPhase(0b010));
    }
    ...
    fn populate_odds_bets(&mut self) {
        ...
        let odds_bets = [
            BetType::OddsPass,
            BetType::OddsDontPass,
            BetType::OddsCome,
            BetType::OddsDontCome,
        ];

        for bet_type in odds_bets {
            self.special_requirements
                .insert(bet_type, SpecialRequirement::RequiresPoint(0)); // Any point
        }
    }
    ...
    fn populate_repeater_bets(&mut self) {
        ...
        for (bet_type, payout) in repeater_bets {
            ...
            self.special_requirements
                .insert(bet_type, SpecialRequirement::RequiresHistory(100)); // Check many rolls
        }
    }
    ...
    fn populate_special_bets(&mut self) {
        // Fire bet
        self.special_requirements
            .insert(BetType::Fire, SpecialRequirement::RequiresFirePoints(4));

        // Bonus bets
        self.special_requirements.insert(
            BetType::BonusSmall,
            SpecialRequirement::RequiresBonusNumbers(0b111110),
        ); // 2-6
        self.special_requirements.insert(
            BetType::BonusTall,
            SpecialRequirement::RequiresBonusNumbers(0b111111000000),
        ); // 8-12
        self.special_requirements.insert(
            BetType::BonusAll,
            SpecialRequirement::RequiresBonusNumbers(0b111111111110),
        ); // 2-12 except 7

        // Other special bets
        self.special_requirements
            .insert(BetType::HotRoller, SpecialRequirement::RequiresStreak(20));
        self.special_requirements
            .insert(BetType::TwiceHard, SpecialRequirement::RequiresHistory(2));
        self.special_requirements
            .insert(BetType::RideLine, SpecialRequirement::RequiresStreak(3));
    }
        ...
        Self {
            cache: lru::LruCache::new(
                std::num::NonZeroUsize::new(1000)
                    .expect("LRU cache size 1000 is a positive constant"),
            ),
            hits: 0,
            misses: 0,
        }
...
impl EfficientBetResolver {
    ...
    pub fn resolve_bets_fast(
        ...
        for &(bet_type, player, amount) in active_bets {
            if let Some(resolution) =
                self.resolve_single_bet_fast(state, bet_type, player, amount, dice_roll)?
            {
    ...
    fn resolve_single_bet_fast(
        ...
        let (resolution_type, payout_multiplier) =
            PAYOUT_LOOKUP_TABLE.lookup_resolution(bet_type, dice_total);
        ...
        match resolution_type {
            ...
            ResolutionType::Lose => Ok(Some(BetResolution::Lost {
                player,
                bet_type,
                amount,
            })),
            ResolutionType::Push => Ok(Some(BetResolution::Push {
                player,
                bet_type,
                amount,
            })),
            ...
            let resolution = match requirement {
                SpecialRequirement::RequiresFirePoints(min_points) => {
                    if state.get_fire_points() >= *min_points {
                        Some(self.create_fire_bet_resolution(
                            player,
                            amount,
                            state.get_fire_points(),
                        ))
                    } else {
                        None
                    }
                ...
                SpecialRequirement::RequiresStreak(min_streak) => {
                    if state.get_hot_streak() >= *min_streak as u16 {
                        Some(self.create_streak_bet_resolution(
                            bet_type,
                            player,
                            amount,
                            state.get_hot_streak(),
                        ))
                    } else {
                        None
                    }
    ...
    fn create_bonus_bet_resolution(
        &self,
        bet_type: BetType,
        player: PeerId,
        amount: CrapTokens,
    ...
    fn create_streak_bet_resolution(
        &self,
        bet_type: BetType,
        player: PeerId,
        amount: CrapTokens,
...
mod tests {
    ...
    fn test_efficient_bet_resolver() {
        ...
        let resolutions = resolver
            .resolve_bets_fast(&state, dice_roll, &active_bets)
            .unwrap();
    ...
    fn test_bet_resolution_caching() {
        ...
        let active_bets = vec![(BetType::Field, [2; 32], CrapTokens::new_unchecked(100))];

        // First call should miss cache
        let _resolutions1 = resolver
            .resolve_bets_fast(&state, dice_roll, &active_bets)
            .unwrap();

        // Second call should hit cache
        let _resolutions2 = resolver
            .resolve_bets_fast(&state, dice_roll, &active_bets)
            .unwrap();
    ...
    fn test_special_bet_resolution() {
        ...
        let resolution = resolver
            .resolve_single_bet_fast(&state, BetType::Fire, player, amount, dice_roll)
            .unwrap();
    ...
    fn test_lookup_table_size() {
        ...
        assert!(
            stats.lookup_table_size < 32 * 1024,
            "Lookup table size {} exceeds 32KB",
            stats.lookup_table_size
        );
    }
