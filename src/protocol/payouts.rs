...
pub trait PayoutCalculator {
    ...
    fn resolve_ride_line_bet(
        &self,
        player: &PeerId,
        bet: &Bet,
        pass_wins: u32,
    ) -> Option<BetResolution>;

    /// Resolve Muggsy bet (7 on comeout or point-7 combination)
    fn resolve_muggsy_bet(
        &self,
        player: &PeerId,
        bet: &Bet,
        last_two_rolls: &[DiceRoll],
    ) -> Option<BetResolution>;

    /// Resolve Replay bet (same point 3+ times)
    fn resolve_replay_bet(
        &self,
        player: &PeerId,
        bet: &Bet,
        point_history: &[u8],
    ...
    fn resolve_different_doubles_bet(
        &self,
        player: &PeerId,
        bet: &Bet,
        doubles_rolled: &HashSet<u8>,
    ) -> Option<BetResolution>;
}
...
impl PayoutCalculator for CrapsGame {
    ...
    fn resolve_bonus_bets(
        ...
        if let Some(bet) = bets.get(&BetType::BonusAll) {
            let all_numbers: HashSet<u8> =
                [2, 3, 4, 5, 6, 8, 9, 10, 11, 12].iter().copied().collect();
    ...
    fn resolve_replay_bet(
        ...
        for (&_point, &count) in &point_counts {
            if count >= 3 {
                ...
                let payout =
                    CrapTokens::new_unchecked((bet.amount.amount() * multiplier as u64) / 100);
                return Some(BetResolution::Won {
                    player: *player,
