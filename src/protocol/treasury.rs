...
impl Treasury {
    ...
    pub fn lock_funds(
        ...
        if max_payout > available {
            return Err(Error::InsufficientFunds(format!(
                "Treasury cannot cover max payout of {} CRAP",
                max_payout.0
            )));
        }
        ...
        if amount > max_bet {
            return Err(Error::InvalidBet(format!(
                "Bet of {} exceeds max allowed bet of {}",
                amount.0, max_bet.0
            )));
        }

        // Lock the maximum potential payout
        self.locked_funds = self
            .locked_funds
        ...
        *self
            .game_locks
            .entry(game_id)
            .or_insert(CrapTokens::from(0)) = current
            .checked_add(max_payout)
    ...
    pub fn settle_bet(
        &mut self,
        game_id: GameId,
        locked_amount: CrapTokens,
        actual_payout: CrapTokens,
    ) -> Result<(), Error> {
        // Get locked amount for this game
        let game_locked = self
            .game_locks
            .get(&game_id)
            .copied()
            .unwrap_or(CrapTokens::from(0));
        if locked_amount > game_locked {
            return Err(Error::InvalidState(format!(
                "Attempted to release {} but only {} locked for game",
        ...
        if actual_payout.0 > 0 {
            if actual_payout > self.balance {
                return Err(Error::InsufficientFunds(format!(
                    "Treasury balance {} insufficient for payout {}",
            ...
            let house_win = locked_amount
                .checked_sub(actual_payout)
                .unwrap_or(locked_amount);
            self.balance = self
                .balance
                .checked_add(house_win)
                .ok_or_else(|| Error::InvalidData("Overflow in balance".to_string()))?;
        }
    ...
    pub fn verify_integrity(&self) -> bool {
        ...
        let total_game_locks: u64 = self.game_locks.values().map(|t| t.0).sum();
...
mod tests {
    ...
    fn test_treasury_health() {
        ...
        treasury
            .lock_funds(game_id, bet_amount, max_payout)
            .unwrap();
