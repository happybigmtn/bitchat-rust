...
use super::{BetType, DiceRoll, GameId, PeerId};
...
impl CompactGameState {
    ...
    pub fn set_player_active(&mut self, player_index: usize, active: bool) {
        ...
        self.player_states[word_index] =
            (self.player_states[word_index] & !mask) | ((new_state & 0xF) << bit_offset);
    }
    ...
    pub fn add_bet_type(&mut self, bet_type: BetType) {
        ...
        if bit_index < 64 {
            ...
            dynamic_data.compressed_bets.bet_mask |=
                1u64.checked_shl(bit_index as u32).unwrap_or(0);
        }
    }
...
mod tests {
    ...
    fn test_compact_game_state_size() {
        ...
        assert!(
            stats.static_bytes <= 128,
            "Static size {} exceeds 128 bytes",
            stats.static_bytes
        );
        assert!(
            stats.compression_ratio < 0.5,
            "Compression ratio {} should be < 0.5",
    ...
    fn test_state_snapshot() {
        ...
        // Add some deltas
        snapshot.add_delta(StateDelta::PhaseChange {
            new_phase: GamePhase::Ended,
        });
        snapshot.add_delta(StateDelta::RollProcessed {
            roll: DiceRoll::new(4, 3).unwrap(),
        });
