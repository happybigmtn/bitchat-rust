...
pub use game_lifecycle::{GameLifecycleManager, ActiveGame, GameCommand};
...
pub enum GameEvent {
    ...
    ConsensusReached { game_id: GameId },
    TreasuryUpdate { balance: u64, rake_collected: u64 },
}
        ...
        match command {
            GameCommand::CreateGame { creator, config } => {
                ...
                if self.config.enable_consensus {
                    self.consensus_coordinator.initialize_game(game_id, vec![creator]).await?;
                }
