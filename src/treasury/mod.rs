...
impl TreasuryManager {
    ...
    pub async fn initialize_treasury(&self, initial_balance: CrapTokens) -> Result<()> {
        ...
        let cold_wallet = TreasuryWallet {
            ...
            balance: CrapTokens::from((initial_balance.0 as f64 * config.cold_wallet_minimum) as u64),
            reserved_balance: CrapTokens::from(0),
            last_rebalance: Self::current_timestamp(),
            transaction_count: 0,
        ...
        let insurance_amount = CrapTokens::from(
            (initial_balance.0 as f64 * 0.1) as u64
        );
        *self.insurance_fund.write().await = insurance_amount;
        
        log::info!("Treasury initialized with {} CRAP across 4 wallets + insurance fund", 
    ...
    pub async fn collect_and_distribute_fees(&self, fees_collected: CrapTokens) -> Result<()> {
        ...
        let reserve_amount = CrapTokens::from(
            (fees_collected.0 as f64 * distribution.treasury_reserve) as u64
        );
        let staking_amount = CrapTokens::from(
            (fees_collected.0 as f64 * distribution.staking_rewards) as u64
        );
        let dev_amount = CrapTokens::from(
            (fees_collected.0 as f64 * distribution.development_fund) as u64
        );
        let community_amount = CrapTokens::from(
            (fees_collected.0 as f64 * distribution.community_fund) as u64
        );
        let burn_amount = CrapTokens::from(
            (fees_collected.0 as f64 * distribution.burn_allocation) as u64
        );
        
        // Distribute to reserves
        {
            let mut reserves = self.reserve_funds.write().await;
            let current = reserves.get("treasury_reserve")
                .copied()
                .unwrap_or(CrapTokens::from(0));
            reserves.insert("treasury_reserve".to_string(), 
                current.checked_add(reserve_amount).unwrap_or(current));
            
            let current_staking = reserves.get("staking_rewards")
                .copied()
                .unwrap_or(CrapTokens::from(0));
            reserves.insert("staking_rewards".to_string(),
                current_staking.checked_add(staking_amount).unwrap_or(current_staking));
            
            let current_dev = reserves.get("development_fund")
                .copied()
                .unwrap_or(CrapTokens::from(0));
            reserves.insert("development_fund".to_string(),
                current_dev.checked_add(dev_amount).unwrap_or(current_dev));
            
            let current_community = reserves.get("community_fund")
                .copied()
                .unwrap_or(CrapTokens::from(0));
            reserves.insert("community_fund".to_string(),
                current_community.checked_add(community_amount).unwrap_or(current_community));
        }
        ...
        log::info!("Distributed {} CRAP in fees: {}% to reserves, {}% to staking, {}% burned",
    ...
    pub async fn create_amm_pool(
        ...
        log::info!("Created AMM pool {} with {:.2} CRAP / {:.2} token_b reserves",
                  hex::encode(&amm_id[..8]),
    ...
    pub async fn swap_tokens(
        ...
        let output_amount = (output_reserve as f64 * input_with_fee) / 
                           (input_reserve as f64 + input_with_fee);
        
        let output_tokens = CrapTokens::from(output_amount as u64);
        
        // Check slippage tolerance
        if output_tokens < min_output {
            return Err(Error::InvalidData("Slippage tolerance exceeded".to_string()));
        }
        ...
        // Update reserves
        if input_token_a {
            amm.token_a_reserve = amm.token_a_reserve.checked_add(input_amount)
                .ok_or_else(|| Error::InvalidData("Reserve overflow".to_string()))?;
            amm.token_b_reserve = amm.token_b_reserve.checked_sub(output_tokens)
                .ok_or_else(|| Error::InvalidData("Insufficient reserves".to_string()))?;
        } else {
            amm.token_b_reserve = amm.token_b_reserve.checked_add(input_amount)
                .ok_or_else(|| Error::InvalidData("Reserve overflow".to_string()))?;
            amm.token_a_reserve = amm.token_a_reserve.checked_sub(output_tokens)
                .ok_or_else(|| Error::InvalidData("Insufficient reserves".to_string()))?;
        }
        
        // Update statistics
        amm.volume_24h = amm.volume_24h.checked_add(input_amount).unwrap_or(amm.volume_24h);
        let fee_amount = CrapTokens::from((input_amount.0 as f64 * amm.swap_fee_rate) as u64);
        amm.fees_collected = amm.fees_collected.checked_add(fee_amount).unwrap_or(amm.fees_collected);
        amm.last_price = amm.token_b_reserve.0 as f64 / amm.token_a_reserve.0 as f64;
        
        log::info!("Swapped {} for {} tokens in AMM {} for trader {:?}",
    ...
    pub async fn get_treasury_stats(&self) -> TreasuryStats {
        ...
        let hot_balance = wallets.values()
            .filter(|w| matches!(w.wallet_type, WalletType::Hot))
            .map(|w| w.balance.0)
            .sum::<u64>();
        let cold_balance = wallets.values()
            .filter(|w| matches!(w.wallet_type, WalletType::Cold))
            .map(|w| w.balance.0)
            .sum::<u64>();
        
        let total_liquidity = amms.values()
...
mod tests {
    ...
    async fn test_amm_creation_and_swap() {
        ...
        let amm_id = treasury.create_amm_pool(token_a, token_b, 0.003).await.unwrap();
        
        let swap_input = CrapTokens::from(1_000_000); // 1 CRAP
        let min_output = CrapTokens::from(9_000_000); // Expect ~10 token_b
        let trader = [1u8; 32];
        
        let output = treasury.swap_tokens(amm_id, true, swap_input, min_output, trader).await.unwrap();
        assert!(output.0 > 0);
    }
}