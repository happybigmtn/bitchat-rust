...
impl PermissionManager {
    ...
    pub async fn request_permission(
        ...
        if self.should_show_rationale(permission)? {
            self.show_permission_rationale(permission, rationale)
                .await?;
        }
    ...
    pub async fn request_permissions(
        ...
        if !to_request.is_empty() {
            ...
            let needs_rationale = to_request
                .iter()
                .any(|p| self.should_show_rationale(*p).unwrap_or(false));

            if needs_rationale {
                self.show_batch_permission_rationale(&to_request, rationale)
                    .await?;
            }
        ...
        F: Fn(Permission, PermissionState) + Send + Sync + 'static,
    {
        let mut callbacks = self
            .callbacks
            .lock()
            .map_err(|_| Error::InvalidState("Failed to acquire callbacks lock".to_string()))?;
    ...
    pub fn create_onboarding_flow(&self) -> PermissionFlow {
        ...
        steps.push(PermissionStep {
            ...
            description:
                "Keep games running even when switching apps, ensuring uninterrupted gameplay."
                    .to_string(),
            required: true,
        });
        ...
        steps.push(PermissionStep {
            ...
            description:
                "Optional permissions for QR scanning, haptic feedback, and screen management."
                    .to_string(),
            required: false,
        });
    ...
    async fn show_permission_rationale(
        &self,
        permission: Permission,
        rationale: &str,
    ) -> Result<()> {
    ...
    async fn show_batch_permission_rationale(
        &self,
        permissions: &[Permission],
        rationale: &str,
    ) -> Result<()> {
        log::info!(
            "Showing batch rationale for {} permissions: {}",
            permissions.len(),
            rationale
        );
        // In real implementation, would show native UI dialog
    ...
    async fn android_request_permission(&self, permission: Permission) -> Result<PermissionState> {
        ...
        match result {
            ...
            _ => Err(Error::Platform(format!(
                "Android permission request failed: {}",
                result
            ))),
        }
    }

    async fn android_request_permissions(
        &self,
        permissions: Vec<Permission>,
    ) -> Result<HashMap<Permission, PermissionState>> {
        ...
        // Convert to C array
        let c_permissions: Vec<*const c_char> =
            permission_cstrs.iter().map(|cstr| cstr.as_ptr()).collect();
        ...
        if result != 0 {
            return Err(Error::Platform(format!(
                "Android batch permission request failed: {}",
                result
            )));
        }
...
impl PermissionManager {
    ...
    async fn ios_request_permission(&self, permission: Permission) -> Result<PermissionState> {
        ...
        match result {
            ...
            1 => Ok(PermissionState::Denied),
            _ => Err(Error::Platform(format!(
                "iOS permission request failed: {}",
                result
            ))),
        }
    }

    async fn ios_request_permissions(
        &self,
        permissions: Vec<Permission>,
    ) -> Result<HashMap<Permission, PermissionState>> {
...
impl Permission {
    ...
    pub fn category(&self) -> PermissionCategory {
        match self {
            ...
            | Permission::BluetoothAdvertise => PermissionCategory::Bluetooth,
            ...
            | Permission::PhotoLibrary => PermissionCategory::Media,
...
mod tests {
    ...
    async fn test_single_permission_request() {
        ...
        let result = manager
            .request_permission(Permission::Camera, "Camera needed for QR code scanning")
            .await;
    ...
    fn test_permission_explanations() {
        ...
        let location_explanation =
            manager.get_permission_explanation(Permission::AccessCoarseLocation);
