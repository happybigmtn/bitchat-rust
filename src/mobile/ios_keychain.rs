...
impl IOSKeychainManager {
    ...
    pub fn store_item(
        ...
        let access_group_cstr = self.access_group.as_ref()
        ...
        #[cfg(target_os = "ios")]
        {
            let result = unsafe {
                ios_keychain_store_item(
                    ...
                    data.as_ptr(),
                    data.len(),
                    access_group_cstr.as_ref().map_or(ptr::null(), |cstr| cstr.as_ptr()),
    ...
    pub fn delete_item(&self, account: &str) -> Result<()> {
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            if result != 0 && result != -25300 { // Ignore "not found" errors
    ...
    pub fn delete_secure_enclave_key(&self, key_tag: &str) -> Result<()> {
        ...
        {
            let result = unsafe {
                ios_keychain_delete_se_key(key_tag_cstr.as_ptr())
            };
            
            if result != 0 && result != -25300 { // Ignore "not found"
    ...
    fn map_keychain_error(&self, error_code: c_int, operation: &str) -> Error {
        ...
        Error::Platform(format!("iOS Keychain {} failed: {} (code: {})", 
...
extern "C" {
    ...
    #[cfg(target_os = "ios")]
    fn ios_keychain_clear_all_items(
        service: *const c_char,
        access_group: *const c_char
    ) -> c_int;
...
mod tests {
    ...
    fn test_secure_enclave_signing() {
        ...
        // Generate key first
        let _se_key = manager.generate_secure_enclave_key(
            key_tag,
            SecureEnclaveKeyType::ECC256,
            KeychainAccessControl::BiometricAny
        ).unwrap();
    ...
    fn test_access_group_keychain() {
        let manager = IOSKeychainManager::new_with_access_group(
            "com.bitcraps.test",
            "group.bitcraps.shared"
        );
        
        assert!(manager.is_ok());
        
        let manager = manager.unwrap();
        assert_eq!(manager.access_group, Some("group.bitcraps.shared".to_string()));
    }
