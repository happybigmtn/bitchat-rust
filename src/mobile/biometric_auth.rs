...
impl BiometricAuthManager {
    ...
    pub async fn authenticate_user(&self, reason: &str) -> Result<UserAuthSession> {
        ...
        match auth_result.status {
            ...
            BiometricAuthStatus::Failed => Err(Error::Authentication(
                "Biometric authentication failed".to_string(),
            )),
            BiometricAuthStatus::Cancelled => Err(Error::Authentication(
                "Authentication cancelled by user".to_string(),
            )),
            BiometricAuthStatus::Error(msg) => Err(Error::Authentication(format!(
    ...
    pub fn create_protected_wallet_key(&self, wallet_id: &str) -> Result<ProtectedKey> {
        ...
        let _encrypted = self
            .auth_impl
            .encrypt_with_biometric(&key_alias, test_data)?;
...
impl BiometricAuth for AndroidBiometricAuth {
            ...
            match availability {
                0 => {
                    ...
                    if result == 0 {
                        ...
                        Ok(BiometricAvailability::NotAvailable(
                            "Unable to get biometric info".to_string(),
                        ))
                    }
                }
                ...
                _ => Ok(BiometricAvailability::NotAvailable(format!(
                    "Unknown status: {}",
                    availability
                ))),
            }
        ...
        #[cfg(target_os = "android")]
        {
            ...
            match result {
                ...
                _ => Ok(BiometricAuthResult {
                    status: BiometricAuthStatus::Error(format!(
                        "Android biometric error: {}",
                        result
                    )),
                    auth_method: AuthenticationMethod::Biometric(BiometricType::Fingerprint),
                    user_identifier: None,
    ...
    fn invalidate_keys(&self) -> Result<()> {
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to invalidate biometric keys: {}",
                    result
                )));
            }
        }
...
impl BiometricAuth for IOSBiometricAuth {
            ...
            match availability {
                0 => {
                    ...
                    if result == 0 {
                        ...
                        Ok(BiometricAvailability::NotAvailable(
                            "Unable to get biometric types".to_string(),
                        ))
                    }
                }
                1 => Ok(BiometricAvailability::NotEnrolled),
    ...
    fn get_biometric_key(&self, key_alias: &str) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to generate iOS biometric key: {}",
                    result
                )));
            }
    ...
    fn encrypt_with_biometric(&self, key_alias: &str, data: &[u8]) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "iOS biometric encryption failed: {}",
                    result
                )));
            }
    ...
    fn decrypt_with_biometric(&self, key_alias: &str, encrypted_data: &[u8]) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "ios")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "iOS biometric decryption failed: {}",
                    result
                )));
            }
    ...
    fn invalidate_keys(&self) -> Result<()> {
        #[cfg(target_os = "ios")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to invalidate iOS biometric keys: {}",
                    result
                )));
            }
        }
