...
use crate::mobile::{BitCrapsError, NetworkStats, PeerInfo};
...
impl IosBlePeripheral {
    ...
    pub fn connect_to_peer(&self, peer_id: &str) -> Result<(), BitCrapsError> {
        ...
        let peer_id_cstr =
            std::ffi::CString::new(peer_id).map_err(|_| BitCrapsError::InvalidInput {
    ...
    pub fn disconnect_from_peer(&self, peer_id: &str) -> Result<(), BitCrapsError> {
        ...
        if result != 1 {
            warn!(
                "iOS BLE disconnection may have failed for peer: {}",
                peer_id
            );
        }
    ...
    pub fn send_data(&self, peer_id: &str, message: &BinaryMessage) -> Result<(), BitCrapsError> {
        ...
        let data = message.to_bytes().map_err(|e| BitCrapsError::CryptoError {
        ...
        let result = unsafe {
            super::ffi::ios_ble_send_data(peer_id_cstr.as_ptr(), data.as_ptr(), data.len() as u32)
        };
        ...
        debug!(
            "Data sent successfully to peer: {} ({} bytes)",
            peer_id,
            data.len()
        );
        Ok(())
    ...
    pub fn handle_data_received(
        &self,
        peer_id: &str,
        data: &[u8],
    ) -> Result<BinaryMessage, BitCrapsError> {
        debug!(
            "Data received from peer: {} ({} bytes)",
            peer_id,
            data.len()
        );

        // Parse the binary message
        let message = BinaryMessage::from_bytes(data).map_err(|e| BitCrapsError::CryptoError {
        ...
        debug!(
            "Message parsed successfully from peer: {} (type: {:?})",
            peer_id, message.message_type
        );
        Ok(message)
    }
    ...
    pub fn cleanup_expired_peers(&self, max_age_sec: u64) -> Result<u32, BitCrapsError> {
        ...
        peers.retain(|_, peer| peer.is_connected || peer.last_activity >= cutoff_time);
...
mod tests {
    ...
    fn test_peer_discovery() {
        ...
        let peer_info = peripheral
            .handle_peer_discovered("test-peer", -50, &[])
            .unwrap();
