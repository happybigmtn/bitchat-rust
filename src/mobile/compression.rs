...
impl MessageCompressor {
    ...
    pub fn new(config: CompressionConfig) -> Self {
        Self {
            ...
            training_samples: Arc::new(Mutex::new(VecDeque::with_capacity(
                config.dictionary.training_sample_size,
            ))),
    ...
    pub async fn stop(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        log::info!(
            "Compression stats: compressed {} messages, avg ratio: {:.1}%, cache hits: {}",
    ...
    pub async fn update_network_conditions(&self, conditions: NetworkConditions) {
        ...
        *self.network_conditions.write().await = Some(conditions);
        log::debug!(
            "Updated network conditions: {} bps, {} ms latency",
            bandwidth,
            latency
        );
    }
    ...
    pub async fn compress(
        ...
        if data.len() < config.min_compress_size || data.len() > config.max_compress_size {
            return Ok(CompressionResult {
                ...
                compressed_size: data.len(),
                ratio: 1.0,
                compression_time: SystemTime::now()
        ...
        // Perform compression
        let result = self
            .compress_with_algorithm(data, algorithm, start_time)
            .await?;
        ...
        if self.config.read().await.performance.enable_caching && result.ratio < 0.8 {
            let data_hash = self.calculate_hash(data);
            self.add_to_cache(data_hash, result.data.clone(), algorithm)
                .await;
        }
    ...
    pub async fn decompress(
        &self,
        data: &[u8],
    ) -> Result<DecompressionResult, Box<dyn std::error::Error>> {
        let start_time = SystemTime::now();
        ...
        if algorithm == CompressionAlgorithm::None {
            return Ok(DecompressionResult {
                ...
                algorithm,
                decompression_time: SystemTime::now()
                    .duration_since(start_time)
                    .unwrap_or(Duration::ZERO),
            });
        }
        ...
        let decompressed = self
            .decompress_with_algorithm(compressed_data, algorithm, dictionary_id)
            .await?;
    ...
    async fn select_algorithm(
        ...
        if let Some(conditions) = self.network_conditions.read().await.as_ref() {
            if conditions.bandwidth_bps < 100_000 {
                // < 100 KB/s
            ...
            } else if conditions.bandwidth_bps > 1_000_000 {
                // > 1 MB/s
    ...
    async fn compress_with_algorithm(
        ...
        let compressed_data = match algorithm {
            ...
            CompressionAlgorithm::Dictionary => self.compress_dictionary(data).await?,
        };

        // Add compression header
        let final_data = self
            .add_compression_header(compressed_data, algorithm, None)
            .await;
    ...
    fn parse_compressed_data<'a>(
        &self,
        data: &'a [u8],
    ) -> Result<(CompressionAlgorithm, &'a [u8], Option<u32>), Box<dyn std::error::Error>> {
        if data.len() < 8 {
    ...
    async fn add_to_cache(&self, hash: u64, data: Bytes, algorithm: CompressionAlgorithm) {
        ...
        if cache.len() >= config.performance.cache_size {
            // Remove oldest entry
            if let Some((&oldest_hash, _)) = cache.iter().min_by_key(|(_, entry)| entry.last_access)
            {
    ...
    async fn update_compression_stats(&self, result: &CompressionResult) {
        ...
        if stats.total_compressed > 1 {
            stats.average_compression_time_ns =
                (stats.average_compression_time_ns * (stats.total_compressed - 1) + time_ns)
                    / stats.total_compressed;
        } else {
    ...
    async fn update_decompression_stats(&self, result: &DecompressionResult) {
        ...
        if stats.total_decompressed > 1 {
            stats.average_decompression_time_ns =
                (stats.average_decompression_time_ns * (stats.total_decompressed - 1) + time_ns)
    ...
    async fn update_algorithm_performance(
        ...
        if let Some(perf) = performance.get_mut(&algorithm) {
            ...
            let bytes_per_second =
                result.original_size as f64 / result.compression_time.as_secs_f64();
            perf.compression_speed =
                perf.compression_speed * (1.0 - alpha) + bytes_per_second * alpha;
    ...
    async fn start_dictionary_training(&self) {
        ...
        let task = tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(
                config.read().await.dictionary.refresh_interval_secs,
            ));
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                if !samples.is_empty() {
                    log::debug!(
                        "Training compression dictionary with {} samples",
                        samples.len()
                    );
    ...
    async fn train_dictionary_static(
        ...
        for (pattern, count) in pattern_counts {
            if count >= 5 && pattern.len() >= 4 {
                // Minimum frequency and size
                dict.insert(
                    id_counter,
                    DictionaryEntry {
    ...
    async fn start_cache_cleanup(&self) {
        ...
        let task = tokio::spawn(async move {
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                cache.retain(|_, entry| entry.last_access > cutoff_time || entry.access_count > 5);
...
impl ContentTypeDetector {
    fn new() -> Self {
        Self {
            json_patterns: vec![b"{", b"}", b"[", b"]", b"\":", b",\""],
            binary_patterns: vec![b"\x00\x00", b"\xFF\xFF", b"\x89PNG"],
            game_patterns: vec![b"state", b"player", b"score", b"game"],
        }
    }
