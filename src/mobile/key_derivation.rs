...
impl KeyDerivationManager {
    ...
    pub fn get_master_key(&self, key_id: &str) -> Result<SecureKey> {
        let mut master_keys = self
            .master_keys
            .lock()
            .map_err(|_| Error::InvalidState("Failed to acquire master keys lock".to_string()))?;
        ...
        if let Some(master_key) = master_keys.get(key_id) {
            ...
            let master_key = MasterKey {
                ...
                key_material: if self.hardware_backed {
                    Vec::new()
                } else {
                    key_material.clone()
                },
    ...
    pub fn derive_key_pbkdf2(
        ...
        let derived_key = match algorithm {
            PbkdfAlgorithm::Pbkdf2Sha256 => {
                ...
                if key_length != 32 {
                    return Err(Error::InvalidInput(
                        "SHA-256 PBKDF2 produces 32-byte keys".to_string(),
                    ));
                }
            ...
            PbkdfAlgorithm::Pbkdf2Sha512 => {
                ...
                if key_length != 64 {
                    return Err(Error::InvalidInput(
                        "SHA-512 PBKDF2 produces 64-byte keys".to_string(),
                    ));
                }

                let key = pbkdf2_hmac_array::<Sha512, 64>(password.as_bytes(), salt, iterations);
    ...
    pub fn derive_key_hkdf(
        ...
        if key_length == 0 || key_length > 255 * 32 {
            return Err(Error::InvalidInput(
                "Invalid HKDF output key length".to_string(),
            ));
        }
        ...
        let mut derived_keys = self
            .derived_keys
            .lock()
            .map_err(|_| Error::InvalidState("Failed to acquire derived keys lock".to_string()))?;
...
mod tests {
    ...
    fn test_pbkdf2_derivation() {
        ...
        let derived_key =
            manager.derive_key_pbkdf2(password, salt, 10000, 32, PbkdfAlgorithm::Pbkdf2Sha256);
    ...
    fn test_argon2_config_validation() {
        ...
        let weak_method = DerivationMethod::Argon2id {
            config: weak_config,
        };
        ...
        let valid_method = DerivationMethod::Argon2id {
            config: valid_config,
        };
