...
impl NetworkOptimizer {
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        log::info!(
            "Starting network optimizer (target: {} KB/s)",
            self.config
                .read()
                .await
    ...
    pub async fn queue_message(
        ...
        if !self
            .check_bandwidth_quota(&message.component, message.data.len())
            .await
        {
            log::warn!(
                "Message {} from component '{}' exceeds bandwidth quota",
                message.id,
                message.component
            );
        }
        ...
        log::debug!(
            "Queued message {} from '{}' with priority {:?} (effective: {})",
            message.id,
            message.component,
            message.priority,
            effective_priority
    ...
    pub async fn update_connection_quality(&self, peer_id: Vec<u8>, quality: ConnectionQuality) {
        self.connection_qualities
            .write()
            .await
            .insert(peer_id, quality);
    ...
    async fn initialize_bandwidth_allocations(&self) {
        ...
        log::info!(
            "Initialized bandwidth allocations for {} components",
            allocations.len()
        );
    }
    ...
    async fn adjust_bandwidth_allocations(
        &self,
        factor: f64,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mut allocations = self.bandwidth_allocations.write().await;
        ...
        let task = tokio::spawn(async move {
            while is_running.load(Ordering::Relaxed) {
                ...
                if let Some(message_item) = next_message {
                    ...
                    if let Ok(permit) = transmission_semaphore.clone().acquire_owned().await {
                        ...
                        let transmission_time =
                            Self::estimate_transmission_time(message_size).await;
                ...
                // Update queue depth metrics
                {
                    ...
                    metrics_guard
                        .queue_depths
                        .insert(MessagePriority::High, high_queue.lock().await.len());
                    metrics_guard
                        .queue_depths
                        .insert(MessagePriority::Normal, normal_queue.lock().await.len());
                    metrics_guard
                        .queue_depths
                        .insert(MessagePriority::Background, low_queue.lock().await.len());
                }
            }
        });
                ...
                // Update bandwidth usage for components (simplified)
                {
                    ...
                    for allocation in allocations.values_mut() {
                        ...
                        allocation.current_usage_bps = (current_bps as f64
                            * (allocation.allocated_bps as f64 / 100000.0))
                            as u32;

                        allocation
                            .usage_history
                            .push_back((SystemTime::now(), allocation.current_usage_bps));
                        ...
                        if allocation.current_usage_bps > allocation.allocated_bps {
                            ...
                            log::warn!(
                                "Component '{}' over bandwidth allocation: {} / {} bps",
                                allocation.component,
                                allocation.current_usage_bps,
                                allocation.allocated_bps
                            );
                        }
                    }
                }
        ...
        let task = tokio::spawn(async move {
            let mut interval = tokio::time::interval(Duration::from_secs(
                config
                    ...
                    .adaptation_interval_secs as u64,
            ));
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                let optimization_applied =
                    Self::analyze_and_optimize(&connection_qualities, &metrics, &config).await;
    ...
    async fn update_network_metrics(&self) {
        ...
        metrics.effective_bandwidth_bps =
            (metrics.total_bytes_sent as f64 / time_window_secs) as u32;
    ...
    async fn estimate_transmission_time(message_size: usize) -> Duration {
        ...
        let transmission_time_ms =
            (message_size as f64 / base_throughput_bps as f64 * 1000.0) as u64;
    ...
    async fn update_connection_qualities_simulation(
        ...
        if qualities.len() < 3 && rand::random::<f64>() < 0.1 {
            ...
            log::debug!(
                "Simulated new connection with {} total connections",
                qualities.len()
            );
        }
    }
    ...
    async fn analyze_and_optimize(
        ...
        if metrics_guard.average_latency_ms > 200 {
            log::info!(
                "High latency detected ({}ms), optimizing connection parameters",
        ...
        let poor_connections = qualities
            .values()
        ...
        if poor_connections > 0 {
            log::info!(
                "Found {} poor quality connections, optimizing",
                poor_connections
            );
            optimization_applied = true;
        }
