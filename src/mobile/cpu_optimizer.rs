...
impl CpuOptimizer {
    ...
    pub fn new(config: CpuOptimizerConfig) -> Self {
        ...
        Self {
            ...
            usage_history: Arc::new(RwLock::new(VecDeque::with_capacity(config.monitoring.history_window_size))),
            temperature_history: Arc::new(RwLock::new(VecDeque::with_capacity(config.monitoring.history_window_size))),
    ...
    pub async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        log::info!("Starting CPU optimizer (target usage: {:.1}%)", 
    ...
    pub async fn stop(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if total_tasks > 0 {
            ...
            log::info!("CPU optimizer final stats: {} tasks processed, avg time: {}ms",
                      total_tasks, avg_time);
        }
    ...
    /// Schedule a CPU task
    pub async fn schedule_task(&self, mut task: CpuTask) -> Result<u64, Box<dyn std::error::Error>> {
        // Assign task ID
        ...
        log::debug!("Scheduled task '{}' with priority {} (effective: {})",
                   task.name, task.priority as u8, effective_priority);
        
        Ok(task.id)
    }
    
    /// Schedule consensus operation
    pub async fn schedule_consensus(&self, item: ConsensusItem) -> Result<u64, Box<dyn std::error::Error>> {
        if !self.config.read().await.consensus_optimization.enabled {
            ...
            let task = CpuTask {
                ...
                deadline: Some(item.created_at + Duration::from_millis(
                    self.config.read().await.consensus_optimization.max_latency_ms
                )),
                cpu_weight: 1.0,
        ...
        if let Some(batch) = batches.back_mut() {
            ...
            if can_add_to_batch {
                ...
                log::debug!("Added consensus item {} to existing batch {} (size: {})",
                           item.id, batch.id, batch.items.len());
                return Ok(item.id);
            }
        }
        ...
        log::debug!("Created new consensus batch {} for item {}", batch_id, item.id);
    ...
    async fn set_throttle_level(&self, level: ThrottleLevel) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn start_monitoring(&self) {
        ...
        let task = tokio::spawn(async move {
            let mut usage_interval = tokio::time::interval(
                Duration::from_millis(config.read().await.monitoring.usage_sample_interval_ms)
            );
            let mut temp_interval = tokio::time::interval(
                Duration::from_millis(config.read().await.monitoring.temperature_interval_ms)
            );
                ...
                if let Some(task) = next_task {
                    ...
                    if let Ok(permit) = task_semaphore.acquire().await {
                        ...
                        tokio::spawn(async move {
                            ...
                            total_time_clone.fetch_add(execution_time.as_millis() as u64, Ordering::Relaxed);
        ...
        let task = tokio::spawn(async move {
            ...
            while is_running.load(Ordering::Relaxed) {
                ...
                let ready_batch = {
                    ...
                    if let Some(batch) = batches.front() {
                        let batch_timeout = Duration::from_millis(
                            config.consensus_optimization.batch_processing.batch_timeout_ms
                        );
                ...
                if let Some(batch) = ready_batch {
                    ...
                    log::debug!("Completed consensus batch {} in {:?}",
                               batch.id, processing_time);
                }
            }
        });
    ...
    async fn process_consensus_batch(batch: &ConsensusBatch) -> Result<(), Box<dyn std::error::Error>> {
        // Simulate batch processing with improved efficiency
        let total_complexity: u32 = batch.items.iter()
            .map(|item| item.complexity as u32)
            .sum();
        ...
        log::debug!("Processed consensus batch with {} items in {:?}",
                   batch.items.len(), processing_time);
