...
impl MobilePerformanceOptimizer {
    pub fn new(config: MobilePerformanceConfig) -> Self {
        ...
        Self {
            ...
            memory_manager: Arc::new(MobileMemoryManager::new(
                config.memory,
                ...
                *battery_info.write().await = battery_info_current.clone();
                ...
                if let Some(ref battery) = battery_info_current {
                    if let Some(power_draw) = battery.power_draw_watts {
                        ...
                        current_metrics.battery_drain_per_hour =
                            power_draw * 100.0 / (battery.level_percent.max(1.0));
                ...
                current_metrics.performance_score =
                    Self::calculate_performance_score(&*current_metrics, &*config.read().await);
                ...
                if rand::random::<f64>() < 0.1 {
                    ...
                    log::info!(
                        "Performance: CPU {:.1}%, Mem {:.1}MB, Battery {:.1}%/h, Score {:.1}",
    ...
    fn calculate_performance_score(
        ...
        let battery_drain_ratio =
            metrics.battery_drain_per_hour / config.target_battery_drain_per_hour;
        ...
        // Latency penalty
        let latency_ratio =
            metrics.avg_consensus_latency_ms as f64 / config.target_consensus_latency_ms as f64;
