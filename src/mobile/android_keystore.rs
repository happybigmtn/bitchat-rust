...
impl AndroidKeystoreManager {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to initialize Android Keystore: error code {}", result
                )));
            }
        }
    ...
    pub fn get_encryption_key(&self, key_alias: &str) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to get encryption key from Android Keystore: error code {}", result
                )));
            }
        ...
        if actual_size == 0 {
            return Err(Error::Crypto("Received empty key from Android Keystore".to_string()));
        }
    ...
    pub fn encrypt_and_store(&self, key_alias: &str, data: &[u8]) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to encrypt data with Android Keystore: error code {}", result
                )));
            }
    ...
    pub fn retrieve_and_decrypt(&self, key_alias: &str, encrypted_data: &[u8]) -> Result<Vec<u8>> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to decrypt data with Android Keystore: error code {}", result
                )));
            }
    ...
    pub fn delete_key(&self, key_alias: &str) -> Result<()> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to delete key from Android Keystore: error code {}", result
                )));
            }
        }
    ...
    pub fn key_exists(&self, key_alias: &str) -> Result<bool> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result < 0 {
                return Err(Error::Crypto(format!(
                    "Failed to check key existence in Android Keystore: error code {}", result
                )));
            }
    ...
    pub fn get_key_attestation(&self, key_alias: &str) -> Result<KeyAttestation> {
        ...
        #[cfg(target_os = "android")]
        {
            ...
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to get key attestation from Android Keystore: error code {}", result
                )));
            }
    ...
    pub fn clear_all_keys(&self) -> Result<()> {
        ...
        {
            let result = unsafe {
                android_keystore_clear_all(keystore_alias_cstr.as_ptr())
            };
            
            if result != 0 {
                return Err(Error::Crypto(format!(
                    "Failed to clear Android Keystore: error code {}", result
                )));
            }
        }
        ...
        #[cfg(not(target_os = "android"))]
        {
            log::debug!("Simulating keystore clear for alias: {}", self.keystore_alias);
        }
