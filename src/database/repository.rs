...
impl UserRepository {
    ...
    pub async fn list_users(&self, limit: usize) -> Result<Vec<User>> {
        self.pool.with_connection(|conn| {
            ...
            users.collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|e| Error::Database(e.to_string()))
        }).await
    }
}
...
impl GameRepository {
    ...
    pub async fn list_active_games(&self, limit: usize) -> Result<Vec<Game>> {
        self.pool.with_connection(|conn| {
            ...
            games.collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|e| Error::Database(e.to_string()))
        }).await
    }
}
...
impl TransactionRepository {
    ...
    pub async fn get_user_transactions(&self, user_id: &str, limit: usize) -> Result<Vec<Transaction>> {
        self.pool.with_connection(|conn| {
            let mut stmt = conn.prepare(
            ...
            let txs = stmt.query_map(params![user_id, user_id, limit], |row| {
                Ok(Transaction {
                    id: row.get(0)?,
...
impl StatsRepository {
    ...
    pub async fn get_player_stats(&self, player_id: &str) -> Result<Option<PlayerStats>> {
        self.pool.with_connection(|conn| {
            ...
            let stats = stmt.query_row(params![player_id], |row| {
                Ok(PlayerStats {
    ...
    pub async fn get_leaderboard(&self, limit: usize) -> Result<Vec<LeaderboardEntry>> {
        self.pool.with_connection(|conn| {
            ...
            entries.collect::<std::result::Result<Vec<_>, _>>()
                .map_err(|e| Error::Database(e.to_string()))
        }).await
    }
}
