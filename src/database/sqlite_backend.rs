...
mod sqlite_impl {
    ...
    impl SqliteBackend {
        ...
        async fn initialize_schema(&self) -> Result<()> {
            // Create a simple schema for basic functionality when sqlite is enabled
            let schema_sql = r#"
...
pub mod sqlite_stub {
    //! SQLite backend stub when feature is disabled
    use crate::database::abstractions::*;
    use crate::error::{Error, Result};
    
    /// Stub SQLite configuration
    #[derive(Debug, Clone)]
    pub struct SqliteConfig {
        pub database_path: String,
    }
    ...
    /// Stub SqliteBackend that returns errors when sqlite feature is disabled
    pub struct SqliteBackend;
    
    impl SqliteBackend {
        pub async fn new(_config: &DatabaseConnection) -> Result<Self> {
            Err(Error::Database("SQLite support disabled - enable 'sqlite' feature".to_string()))
        }
    }
}
