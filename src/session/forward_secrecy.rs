...
impl ForwardSecrecyManager {
    ...
    pub fn perform_rekey(
        &mut self,
        session: &mut super::noise::NoiseSession,
    ) -> Result<BitchatKeypair> {
        // Check if session is in transport mode
        if !session.is_handshake_finished() {
            return Err(Error::InvalidState(
                "Session not in transport mode for rekeying".to_string(),
            ));
        }
    ...
    fn rekey_transport_state(
        &mut self,
        session: &mut super::noise::NoiseSession,
        _new_keypair: &BitchatKeypair,
    ) -> Result<()> {
        match &mut session.state {
            ...
            _ => Err(Error::InvalidState(
                "Session not in transport ready state".to_string(),
            )),
        }
    }
