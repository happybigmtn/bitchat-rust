...
use crate::protocol::{CrapTokens, Hash256, PeerId};
...
impl PersistentLedger {
    ...
    pub fn new() -> Self {
        Self {
            ...
            treasury_balance: CrapTokens::new_unchecked(
                crate::protocol::treasury::INITIAL_TREASURY_BALANCE,
            ),
            transactions: BTreeMap::new(),
            last_checkpoint: None,
            block_height: 0,
            total_supply: CrapTokens::new_unchecked(
                crate::protocol::treasury::INITIAL_TREASURY_BALANCE,
            ),
            state_root: [0; 32],
        }
        ...
        match &tx.tx_type {
            TransactionType::Genesis { amount } => {
                ...
                if self.block_height != 0 {
                    return Err(Error::InvalidTransaction(
                        "Genesis only allowed at block 0".into(),
                    ));
                }
            ...
            TransactionType::Transfer { from, to, amount } => {
                ...
                *self
                    .balances
                    .entry(*from)
                    .or_insert(CrapTokens::new_unchecked(0)) = from_balance
                    .checked_sub(*amount)
                    .ok_or_else(|| Error::InsufficientFunds("Insufficient balance".into()))?;
                let current = self
                    .balances
                    .entry(*to)
                    .or_insert(CrapTokens::new_unchecked(0));
                *current = current
                    .checked_add(*amount)
            ...
            TransactionType::BetWon { amount, .. } => {
                ...
                if self.treasury_balance < *amount {
                    return Err(Error::InsufficientFunds(
                        "Treasury balance insufficient".into(),
                    ));
                }
                self.treasury_balance =
                    self.treasury_balance.checked_sub(*amount).ok_or_else(|| {
                        Error::InsufficientFunds("Treasury balance insufficient".into())
                    })?;
            }
    ...
    pub fn create_checkpoint(&mut self) {
        ...
        let cutoff = self
            .block_height
            .saturating_sub(MAX_TRANSACTION_HISTORY as u64);
    ...
    pub fn verify_integrity(&self) -> bool {
        ...
        let total_in_wallets: u64 = self.balances.values().map(|t| t.amount()).sum();
...
mod tests {
    ...
    fn test_ledger_operations() {
        ...
        assert_eq!(
            ...
            CrapTokens::from(crate::protocol::treasury::INITIAL_TREASURY_BALANCE)
        );
