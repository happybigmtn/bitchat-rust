...
use bitcraps::protocol::{BitchatPacket, DiceRoll, BetType, Bet, PeerId, GameId};
use bitcraps::crypto::{BitchatIdentity, GameCrypto};
use bitcraps::protocol::consensus::byzantine_engine::{
    ByzantineConsensusEngine, ByzantineConfig, ProposalData
};
use bitcraps::mesh::MeshService;
use bitcraps::transport::{TransportCoordinator, TransportAddress};
use std::sync::Arc;
...
fn bench_consensus_operations(c: &mut Criterion) {
    ...
    group.bench_function("byzantine_consensus_round", |b| {
        b.to_async(&rt).iter(|| async {
            let engine = ByzantineConsensusEngine::new(
                config.clone(),
                crypto.clone(),
                node_id
            );
    ...
    for user_count in [10, 50, 100, 200, 500].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &users| {
                ...
                b.to_async(&rt).iter(|| async {
                    use bitcraps::gaming::{GameSessionManager, GameSessionConfig};
                    ...
                    for i in 0..users {
                        ...
                        let handle = tokio::spawn(async move {
                            ...
                            let session = mgr.create_or_join_session(
                                game_id,
                                player_id,
                                config
                            ).await.unwrap();
