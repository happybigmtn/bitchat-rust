...
use bitcraps::{
    protocol::{PeerId, GameId, DiceRoll},
    protocol::consensus::{ConsensusEngine, ConsensusConfig},
    crypto::{BitchatKeypair, BitchatIdentity},
};
...
fn benchmark_consensus_operations(c: &mut Criterion) {
    ...
    let participants: Vec<PeerId> = (0..5).map(|i| {
        let mut peer = [0u8; 32];
        peer[0] = i;
        peer
    }).collect();
    ...
    group.bench_function("proposal_submission", |b| {
        ...
        b.iter(|| {
            let operation = bitcraps::protocol::consensus::GameOperation::UpdateBalances {
                changes: [(participants[0], bitcraps::protocol::CrapTokens::new(100))].into_iter().collect(),
...
fn benchmark_serialization_operations(c: &mut Criterion) {
    ...
    let config = ConsensusConfig::default();
    let game_id: GameId = [1u8; 16];
    let participants: Vec<PeerId> = (0..10).map(|i| {
        let mut peer = [0u8; 32];
        peer[0] = i;
        peer
    }).collect();
