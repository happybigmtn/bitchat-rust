...
use dashmap::DashMap;
use bitcraps::memory_pool::{MemoryPool, GameMemoryPools};
...
fn bench_concurrent_access(c: &mut Criterion) {
    ...
    let rwlock_map: Arc<RwLock<HashMap<u32, String>>> = 
...
fn bench_stun_selection(c: &mut Criterion) {
    ...
    group.bench_function("parallel_stun", |b| {
        b.to_async(&rt).iter(|| async {
            let tasks: Vec<_> = servers.iter().take(3).map(|(name, delay, _)| {
                let delay = *delay;
