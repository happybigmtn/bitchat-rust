...
fn benchmark_packet_operations(c: &mut Criterion) {
    ...
    group.bench_function("packet_creation", |b| {
        b.iter(|| {
            let _packet = BitchatPacket::new_ping(
                black_box(source), 
                black_box(target)
            );
        })
    ...
    group.bench_function("anti_cheat_validation", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let _result = detector.validate_bet(black_box(&bet), black_box(&player)).await;
            })
        })
    });
...
fn benchmark_memory_patterns(c: &mut Criterion) {
    ...
    group.bench_function("frequent_allocation", |b| {
        b.iter(|| {
            let _data: Vec<Vec<u8>> = (0..100)
                .map(|_| vec![0u8; black_box(64)])
                .collect();
        })
    });
    ...
    group.bench_function("arc_clone_operations", |b| {
        ...
        b.iter(|| {
            let _clones: Vec<_> = (0..100)
                .map(|_| Arc::clone(black_box(&data)))
                .collect();
        })
    });
