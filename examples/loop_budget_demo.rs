...
use bitcraps::utils::{BoundedLoop, CircuitBreaker, LoadShedder, LoopBudget, OverflowHandler};
use std::sync::Arc;
...
async fn basic_loop_budget_example() {
    ...
    for _ in 0..150 {
        if !budget.can_proceed() {
            println!(
                "Budget exhausted at iteration {}, backing off...",
                iterations
            );
...
async fn bounded_message_processing_example() {
    ...
    let processor = tokio::spawn(async move {
        bounded_loop
            .process_with_budget(|message| async move {
        ...
        println!(
            "Total iterations: {}",
            stats.iterations.load(std::sync::atomic::Ordering::Relaxed)
        );
        println!(
            "Budget exceeded: {}",
            stats
...
async fn network_loop_example() {
    ...
    let (tx, mut rx) = mpsc::channel::<i32>(1000); // Bounded at 1000 messages
    ...
    let consumer = tokio::spawn(async move {
        ...
        loop {
            ...
            if !budget.can_proceed() {
                println!(
                    "Budget exhausted (processed: {}), backing off...",
                    processed_count
                );
                budget.backoff().await;
                continue;
            ...
            match tokio::time::timeout(Duration::from_millis(100), rx.recv()).await {
                Ok(Some(message)) => {
                    ...
                    if processed_count % 100 == 0 {
                        println!(
                            "Processed {} messages, budget utilization: {:.1}%",
                            processed_count,
                            budget.utilization()
                        );
                    }
                ...
                Ok(None) => {
                    println!(
                        "Channel closed, processed {} total messages",
                        processed_count
                    );
                    break;
                }
...
async fn consensus_loop_example() {
    ...
    while round < 1000 && start.elapsed() < Duration::from_secs(10) {
        if !budget.can_proceed() {
            println!(
                "Consensus budget exhausted at round {}, backing off...",
                round
            );
            budget.backoff().await;
            continue;
