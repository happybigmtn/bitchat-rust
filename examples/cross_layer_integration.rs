...
use bitcraps::transport::{Transport, BluetoothTransport, TransportPacket};
use bitcraps::mesh::{MeshService, MeshPacket};
use bitcraps::protocol::{BitchatPacket, PeerId};
use bitcraps::crypto::{encrypt_message, decrypt_message, Identity};
...
async fn demonstrate_layer_flow() -> Result<()> {
    ...
    let protocol_msg = BitchatPacket::create_message(
        sender.peer_id(),
        receiver.peer_id(),
        game_action.to_vec(),
    );
    ...
    println!("   Overhead: {} bytes", encrypted.len() - protocol_msg.payload.len());
    println!();
    ...
    println!("   Fragmentation: {}", 
        if transport_packet.payload.len() > 512 { "Required" } else { "Not needed" });
    println!();
