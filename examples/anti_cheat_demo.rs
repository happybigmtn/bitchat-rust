...
use bitcraps::protocol::anti_cheat::{
    AntiCheatEngine, AntiCheatConfig, CheatEvidence, CheatType,
    RandomnessValidator, PeerBehaviorProfile
};
use bitcraps::protocol::{PeerId, GameId, DiceRoll};
...
fn main() -> Result<()> {
    ...
    println!("  Suspicious activities: {}", cheater_profile.suspicious_activities.len());
    ...
    for (i, bet) in bets.iter().enumerate() {
        println!("  Bet {}: {} tokens (outcome: {:?})", 
            i + 1, bet.amount.0, bet.outcome);
    }
