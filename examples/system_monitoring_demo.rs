...
use bitcraps::monitoring::{
    global_system_monitor, metrics::METRICS
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let monitoring_task = bitcraps::monitoring::metrics::MetricsCollector::start_system_monitoring();
    ...
    for i in 1..=10 {
        ...
        match monitor.collect_metrics() {
            Ok(metrics) => {
                ...
                println!("💾 Memory: {:.1} MB used / {:.1} MB total ({:.1} MB available)",
                ...
                let active_interfaces: Vec<_> = metrics.network_interfaces
                    .iter()
                ...
                println!("📈 Global metrics updated: {}", METRICS.is_real_system_monitoring());
