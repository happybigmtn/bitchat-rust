...
use bitcraps::protocol::efficient_consensus::{ConsensusConfig, EfficientDiceConsensus};
...
fn demo_compact_game_state() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ 40,000 field accesses in {:?} ({:.1} ns/access)",
        access_time,
        access_time.as_nanos() as f64 / 40000.0
    );
    ...
    snapshot.add_delta(
        bitcraps::protocol::efficient_game_state::StateDelta::RollProcessed {
...
fn demo_consensus_mechanisms() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut consensus =
        EfficientDiceConsensus::new(game_id, participants.clone(), ConsensusConfig::default());
...
fn demo_history_storage() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ Recent access time: {:.2} μs",
        metrics.recent_access_time_us
    );
