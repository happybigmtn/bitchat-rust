...
use bitcraps::transport::{
    BleTransportInitializer, BleConfigBuilder, AdvertisingConfig,
    TransportEvent, BITCRAPS_SERVICE_UUID
};
...
async fn basic_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let initializer = BleTransportInitializer::new(local_peer_id, 
    ...
    coordinator.start_ble_advertising(initializer.config.advertising.clone()).await?;
    
    println!("✅ BLE advertising started successfully");
    println!("📡 Advertising as: {}", initializer.config.advertising.local_name);
    println!("🔧 Service UUID: {}", initializer.config.advertising.service_uuid);
        ...
        if let Some(event) = coordinator.next_event().await {
            match event {
                TransportEvent::Connected { peer_id, address } => {
                    println!("📱 Central connected: {:?} from {:?}", 
                            hex::encode(peer_id), address);
                }
                TransportEvent::Disconnected { peer_id, reason } => {
                    println!("📱 Central disconnected: {:?} ({})", 
                            hex::encode(peer_id), reason);
                }
                TransportEvent::DataReceived { peer_id, data } => {
                    println!("📥 Received {} bytes from {:?}: {:?}", 
                            data.len(), hex::encode(peer_id), 
                            String::from_utf8_lossy(&data));
                }
                TransportEvent::Error { peer_id, error } => {
                    println!("❌ Transport error from {:?}: {}", 
                            peer_id.map(hex::encode), error);
                }
            }
        }
    ...
    if let Ok(stats) = coordinator.get_enhanced_bluetooth_stats().await {
        ...
        println!("  - Advertising duration: {:?}", stats.peripheral_stats.advertising_duration);
    }
...
async fn advanced_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::spawn(async move {
        loop {
            ...
            if !peers.is_empty() {
                let test_message = format!("Hello from BitChat! Time: {}", 
                ...
                for peer_id in peers {
                    if let Err(e) = coordinator.send_to_peer(peer_id, test_message.as_bytes().to_vec()).await {
    ...
    while start_time.elapsed() < timeout {
        if let Some(event) = coordinator.next_event().await {
            match event {
                TransportEvent::Connected { peer_id, address } => {
                    peer_count += 1;
                    println!("🤝 Peer connected: {:?} from {:?} (total: {})", 
                            hex::encode(peer_id), address, peer_count);
                }
                TransportEvent::Disconnected { peer_id, reason } => {
                    peer_count = peer_count.saturating_sub(1);
                    println!("👋 Peer disconnected: {:?} ({}) (remaining: {})", 
                            hex::encode(peer_id), reason, peer_count);
                }
                TransportEvent::DataReceived { peer_id, data } => {
                    message_count += 1;
                    println!("📨 Message #{} from {:?}: {}", 
                            message_count, hex::encode(peer_id), 
                            String::from_utf8_lossy(&data));
                }
                TransportEvent::Error { peer_id, error } => {
                    println!("❌ Network error from {:?}: {}", 
                            peer_id.map(hex::encode), error);
                }
            }
        }
        ...
        if start_time.elapsed().as_secs() % 10 == 0 {
            if let Ok(stats) = coordinator.get_enhanced_bluetooth_stats().await {
                println!("📊 Status: {} connections, {} sent, {} received", 
    ...
    if let Ok(stats) = coordinator.get_enhanced_bluetooth_stats().await {
        ...
        println!("  - Messages received: {} bytes", stats.total_bytes_received);
        println!("  - Central connections: {}", stats.central_connections);
        println!("  - Peripheral connections: {}", stats.peripheral_stats.active_connections);
        println!("  - Total advertising time: {:?}", stats.peripheral_stats.advertising_duration);
...
async fn platform_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("BLE Peripheral Support: {}", capabilities.supports_peripheral);
    ...
    #[cfg(target_os = "android")]
    {
        ...
        config.platform_settings.android.request_battery_optimization_exemption = true;
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        println!("  - Bluetooth adapter: {}", config.platform_settings.linux.adapter_name);
        println!("  - Auto power-on: enabled");
        println!("  - D-Bus timeout: {:?}", config.platform_settings.linux.dbus_timeout);
    }
