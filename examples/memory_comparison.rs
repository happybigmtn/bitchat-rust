...
fn main() {
    ...
    println!(
        "  Gateway buffer:     {} KB",
        old_gateway_buffer.capacity() / 1024
    );
    println!(
        "  Noise write buffer: {} KB",
        old_noise_write_buffer.capacity() / 1024
    );
    println!(
        "  Noise read buffer:  {} KB",
        old_noise_read_buffer.capacity() / 1024
    );
    println!(
        "  Keychain buffer:    {} KB",
        old_keychain_buffer.capacity() / 1024
    );
    ...
    println!(
        "  After 3KB usage:  {} bytes (high water mark: {})",
        adaptive_buffer.capacity(),
        adaptive_buffer.high_water_mark()
    );
    ...
    println!(
        "  After shrinkage:  {} bytes (optimized for actual usage)",
        adaptive_buffer.capacity()
    );

    println!(
        "\n✅ Memory optimization successfully reduces usage by {:.1}%",
        savings_percentage
    );
...
fn simulate_typical_usage(buffer: &mut GrowableBuffer, name: &str, typical_size: usize) {
    ...
    println!(
        "  {} buffer:     {} KB (high water mark: {} bytes)",
        name,
