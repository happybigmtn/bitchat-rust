...
use bitcraps::mesh::{MeshService, MeshConfig, MeshPeer};
use bitcraps::transport::{TransportCoordinator, MockTransport};
...
async fn main() -> Result<()> {
    ...
    let message = BitchatPacket::create_message(
        *sender_id,
        *target_id,
        b"Hello from the mesh!".to_vec(),
    );
